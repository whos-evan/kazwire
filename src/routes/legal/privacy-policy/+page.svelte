...
<article class="prose lg:prose-lg">
	<h1>Privacy Policy for {config.branding.name}</h1>
	<p>
	...
	<p>
		...
		website with regards to the information that they shared and/or collect in {config.branding
	...
	<h2>Log Files</h2>
	<p>
		{config.branding.name} follows a standard procedure of using log files. These files log visitors
		when they visit websites. All hosting companies do this as part of hosting services' analytics. The
	...
	<p>
		Like any other website, {config.branding.name} uses 'cookies'. These cookies are used to store information
	...
	<p>
		...
		that are used in their respective advertisements and links that appear on {config.branding
			.name}, which are sent directly to users' browsers. They automatically receive your IP address
	...
	<p>
		Note that {config.branding.name} has no access to or control over these cookies that are used by
		third-party advertisers.
	</p>
	<h2>Third Party Privacy Policies</h2>
	<p>
		{config.branding.name}'s Privacy Policy does not apply to other advertisers or websites. Thus,
	...
	<p>
		{config.branding.name} does not knowingly collect any Personal Identifiable Information from children

...
{#if !$isLoading}
	<div class="mb-6 flex justify-center">
		<form
			class="flex w-full max-w-[24rem] flex-col justify-center space-y-2 rounded-lg bg-none md:flex-row md:space-y-0 md:bg-white"
		...
		>
			...
			<input
				...
				type="text"
				placeholder={$_('pages.apps.search_placeholder')}
	...
	>
		{#if data.apps.length === 0}
			...
			<h1 class="col-span-12 text-center text-3xl text-white">{$_('pages.apps.no_results')}</h1>
		{:else}

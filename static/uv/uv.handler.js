...
async function __uvHook(window, config = {}, bare = '/bare/') {
	...
	const __uv = new Ultraviolet({ ...config, window });
	...
	if (window.localStorage) {
		for (const key in window.localStorage) {
			if (key.startsWith(methodPrefix + __uv.location.origin + '@')) {
				__uv.localStorageObj[key.slice((methodPrefix + __uv.location.origin + '@').length)] =
	...
	if (window.sessionStorage) {
		for (const key in window.sessionStorage) {
			if (key.startsWith(methodPrefix + __uv.location.origin + '@')) {
				__uv.sessionStorageObj[key.slice((methodPrefix + __uv.location.origin + '@').length)] =
	...
	client.element.hookProperty([HTMLImageElement], 'srcset', {
		get: (target, that) => {
			return (
				client.element.getAttribute.call(that, __uv.attributePrefix + '-attr-srcset') ||
				target.call(that)
			);
		},
		set: (target, that, [val]) => {
	...
	client.element.hookProperty(HTMLIFrameElement, 'srcdoc', {
		get: (target, that) => {
			return (
				client.element.getAttribute.call(that, __uv.attributePrefix + '-attr-srcdoc') ||
				target.call(that)
			);
		},
		set: (target, that, [val]) => {
	...
	client.document.on('setDomain', (event) => {
		if (
			!event.data.value.toString().endsWith(__uv.meta.url.hostname.split('.').slice(-2).join('.'))
		)
	...
	client.style.on('setProperty', (event) => {
		if (client.style.dashedUrlProps.includes(event.data.property)) {
			event.data.value = __uv.rewriteCSS(event.data.value, { context: 'value', ...__uv.meta });
		}
	});
	...
	if ('CSS2Properties' in window) {
		for (const key of client.style.urlProps) {
			client.overrideDescriptor(window.CSS2Properties.prototype, key, {
				get: (target, that) => {
					return __uv.sourceCSS(target.call(that), { context: 'value', ...__uv.meta });
				},
				set: (target, that, val) => {
					target.call(that, __uv.rewriteCSS(val, { context: 'value', ...__uv.meta }));
				}
			});
		}
					...
					for (const key of client.style.urlProps) {
						client.nativeMethods.defineProperty(value, key, {
							...
							get() {
								...
								return __uv.sourceCSS(value, { context: 'value', ...__uv.meta });
							},
							set(val) {
								client.style.setProperty.call(
									this,
									client.style.propToDashed[key] || key,
									__uv.rewriteCSS(val, { context: 'value', ...__uv.meta })
								);
							}
						});

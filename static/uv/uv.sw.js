...
class UVServiceWorker extends EventEmitter {
		...
		try {
			...
			if (request.referrer && request.referrer.startsWith(location.origin)) {
				...
				if (
					...
					(ultraviolet.meta.url.origin !== referer.origin && request.mode === 'cors')
				) {
			...
			if (responseCtx.headers['set-cookie']) {
				Promise.resolve(
					ultraviolet.cookie.setCookies(responseCtx.headers['set-cookie'], db, ultraviolet.meta)
				).then(() => {
	...
	getBarerResponse(response) {
		...
		return {
			...
			body: !this.statusCode.empty.includes(+response.headers.get('x-bare-status'))
				? response.body
				: null
		};
	}
...
class RequestContext {
	...
	get send() {
		return new Request(
			!this.blob ? this.address.href + 'v1/' : 'blob:' + location.origin + this.url.pathname,
			{
				method: this.method,
				headers: {
...
function isHtml(url, contentType = '') {
	return (
		(Ultraviolet.mime.contentType(contentType || url.pathname) || 'text/html').split(';')[0] ===
		'text/html'
	);
}
class HookEvent {

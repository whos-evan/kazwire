...
function pauseCircularEnemy_1_4(
	centerX,
	centerY,
	radius,
	startAngle,
...
function resetEnemies(l) {
	for (var i = 0; i < enemies[l].length; i++) {
		if (enemies[l][i].enemyType == 'simple') {
			...
			enemies[l][i].x =
				enemies[l][i].radius * Math.cos(enemies[l][i].angle * (Math.PI / 180)) +
				enemies[l][i].centerX;
			enemies[l][i].y =
				enemies[l][i].radius * Math.sin(enemies[l][i].angle * (Math.PI / 180)) +
				enemies[l][i].centerY;
			enemies[l][i].simpleX = enemies[l][i].x / 40;
			enemies[l][i].simpleY = enemies[l][i].y / 40;
		}
...
function drawEnemies() {
	if (state == 'game') {
		...
		for (var i = 0; i < enemies[level].length; i++) {
			canvas.beginPath();
			canvas.arc(
				cwh(enemies[level][i].x) + os.x,
				cwh(enemies[level][i].y) + os.y,
				cwh(ENEMY_SIZE) / 2,
				0,
				2 * Math.PI,
				false

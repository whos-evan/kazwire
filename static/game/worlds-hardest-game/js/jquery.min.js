...
(function (E, B) {
	...
	function da(a, b, d, e) {
		if (c.isArray(b) && b.length)
			c.each(b, function (f, h) {
				d || Ra.test(a)
					? e(a, h)
					: da(a + '[' + (typeof h === 'object' || c.isArray(h) ? f : '') + ']', h, d, e);
			});
		...
		c = (function () {
			...
			b.extend({
				...
				uaMatch: function (j) {
					j = j.toLowerCase();
					j =
						L.exec(j) || g.exec(j) || i.exec(j) || (j.indexOf('compatible') < 0 && n.exec(j)) || [];
			...
			if (a && typeof a === 'string')
				for (var b = (a || '').split(ha), d = 0, e = this.length; d < e; d++) {
					...
					if (f.nodeType === 1)
						if (f.className) {
							for (
								var h = ' ' + f.className + ' ', l = f.className, k = 0, o = b.length;
								k < o;
								k++
							)
			...
			if ((a && typeof a === 'string') || a === B)
				for (var b = (a || '').split(ha), d = 0, e = this.length; d < e; d++) {
					...
					if (f.nodeType === 1 && f.className)
						if (a) {
							for (
								var h = (' ' + f.className + ' ').replace(sa, ' '), l = 0, k = b.length;
								l < k;
								l++
							)
								h = h.replace(' ' + b[l] + ' ', ' ');
				...
				if (b) {
					...
					if (c.nodeName(b, 'select')) {
						...
						for (e = b ? e + 1 : f.length; h < e; h++) {
							...
							if (
								l.selected &&
								(c.support.optDisabled ? !l.disabled : l.getAttribute('disabled') === null) &&
								(!l.parentNode.disabled || !c.nodeName(l.parentNode, 'optgroup'))
							) {
								a = c(l).val();
								if (b) return a;
								d.push(a);
	...
	c.event = {
		...
		trigger: function (a, b, d, e) {
			...
			a.currentTarget = d;
			(e = d.nodeType ? c.data(d, 'handle') : (c.data(d, '__events__') || {}).handle) &&
				e.apply(d, b);
			...
			if (D.length > 1 && x.exec(g))
				...
				if (i) {
					q = m
						...
						: k.find(
								D.pop(),
								D.length === 1 && (D[0] === '~' || D[0] === '+') && i.parentNode ? i.parentNode : i,
								O
						  );
							...
							b
								? c.nodeName(this[f], 'table')
									? this[f].getElementsByTagName('tbody')[0] ||
									  this[f].appendChild(this[f].ownerDocument.createElement('tbody'))
									: this[f]
								: this[f],
	...
	c.extend({
				...
				for (h = 0; f[h]; h++)
					if (
						...
						f[h].nodeType === 1 &&
							f.splice.apply(
								f,
								[h + 1, 0].concat(c.makeArray(f[h].getElementsByTagName('script')))
							);
						d.appendChild(f[h]);
					}
			return f;
		...
		cleanData: function (a) {
			for (
				var b, d, e = c.cache, f = c.event.special, h = c.support.deleteExpando, l = 0, k;
				(k = a[l]) != null;
				l++
			)
	...
	if (c.expr && c.expr.filters) {
		c.expr.filters.hidden = function (a) {
			...
			return (
				(a.offsetWidth === 0 && b === 0) ||
				(!c.support.reliableHiddenOffsets && (a.style.display || c.css(a, 'display')) === 'none')
			);
		};
	...
	c.fx.prototype = {
		...
		step: function (a) {
			...
			if (a || b >= this.options.duration + this.startTime) {
				...
				this.pos = c.easing[
					(this.options.specialEasing && this.options.specialEasing[this.prop]) || b
		...
		step: {
			...
			_default: function (a) {
				if (a.elem.style && a.elem.style[a.prop] != null)
					a.elem.style[a.prop] =
						(a.prop === 'width' || a.prop === 'height' ? Math.max(0, a.now) : a.now) + a.unit;
			...
			if (e !== B)
				...
				return (h = fa(f))
					? 'pageXOffset' in h
						...
						: (c.support.boxModel && h.document.documentElement[d]) || h.document.body[d]
					: f[d];
		};
	});
	c.each(['Height', 'Width'], function (a, b) {
		...
		c.fn[d] = function (e) {
			...
			if (c.isWindow(f))
				return (
					(f.document.compatMode === 'CSS1Compat' && f.document.documentElement['client' + b]) ||

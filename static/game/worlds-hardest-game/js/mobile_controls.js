...
if (mobile) {
		...
		function (event) {
			if (state == 'game' && !paused && !hideKeys) {
				for (var i = 0; i < event.targetTouches.length; i++) {
					// down
					if (
						event.targetTouches[i].pageX > cwh(coords_down[0]) + os.x &&
					...
					// right
						...
						event.targetTouches[i].pageX > cwh(coords_right[0]) + os.x &&
					...
					// up
						...
						event.targetTouches[i].pageX > cwh(coords_up[0]) + os.x &&
					...
					// left
						...
						event.targetTouches[i].pageX > cwh(coords_left[0]) + os.x &&
		...
		function (event) {
			if (state == 'game' && !paused && !hideKeys) {
				for (var i = 0; i < event.changedTouches.length; i++) {
					// down
					if (
						event.changedTouches[i].pageX > cwh(coords_down[0]) + os.x &&
					...
					// right
						...
						event.changedTouches[i].pageX > cwh(coords_right[0]) + os.x &&
					...
					// up
						...
						event.changedTouches[i].pageX > cwh(coords_up[0]) + os.x &&
					...
					// left
						...
						event.changedTouches[i].pageX > cwh(coords_left[0]) + os.x &&
	...
	function drawMobileControls() {
		if (!hideKeys) {
			...
			// down
			canvas.beginPath();
			canvas.rect(cwh(580) + os.x, cwh(440) + os.y, cwh(size), cwh(size));
			if (keyDown) {
			...
			// right
			canvas.beginPath();
			canvas.rect(cwh(690) + os.x, cwh(440) + os.y, cwh(size), cwh(size));
			if (keyRight) {
				canvas.fillStyle = keyFillOn;
			} else {
				canvas.fillStyle = keyFill;
			...
			// up
			canvas.beginPath();
			canvas.rect(cwh(580) + os.x, cwh(330) + os.y, cwh(size), cwh(size));
			if (keyUp) {
				canvas.fillStyle = keyFillOn;
			} else {
				canvas.fillStyle = keyFill;
			...
			// left
			canvas.beginPath();
			canvas.rect(cwh(470) + os.x, cwh(440) + os.y, cwh(size), cwh(size));
			if (keyLeft) {
				canvas.fillStyle = keyFillOn;
			} else {
				canvas.fillStyle = keyFill;
	...
	if (state == 'game' && !paused && !hideKeys) {
		if (touchOn) {
			...
			touchDistance = Math.sqrt(
				Math.pow(touchStartX - touchMoveX, 2) + Math.pow(touchStartY - touchMoveY, 2)
			);
			...
			// arrow - right
			if (
				touchDistance >= cwh(touchSensitivity) &&
				(touchAngle >= 360 - 45 / 2 || touchAngle < 45 / 2)
			) {
				...
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize - arrowPointSize),
					touchStartY - cwh(arrowPointSize)
				);
				canvas.lineTo(touchStartX + cwh(arrowMainSize), touchStartY);
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize - arrowPointSize),
			...
			// arrow - left
				...
				touchDistance >= cwh(touchSensitivity) &&
				touchAngle >= 45 * 3 + 45 / 2 &&
				touchAngle < 45 * 4 + 45 / 2
			) {
			...
			// arrow - up
				...
				touchDistance >= cwh(touchSensitivity) &&
				touchAngle >= 45 * 5 + 45 / 2 &&
				touchAngle < 45 * 6 + 45 / 2
			) {
				...
				canvas.lineTo(
					touchStartX - cwh(arrowPointSize),
					touchStartY - cwh(arrowMainSize - arrowPointSize)
				);
				canvas.lineTo(touchStartX, touchStartY - cwh(arrowMainSize));
				canvas.lineTo(
					touchStartX + cwh(arrowPointSize),
			...
			// arrow - down
				...
				touchDistance >= cwh(touchSensitivity) &&
				touchAngle >= 45 * 1 + 45 / 2 &&
				touchAngle < 45 * 2 + 45 / 2
			) {
				//else if ((touchMoveY >= touchStartY + cwh(touchSensitivity)) &&
				//        !(touchMoveX >= touchStartX + cwh(touchSensitivity) + cwh(diagSensitivity)) &&
				//        !(touchMoveX <= touchStartX - cwh(touchSensitivity) - cwh(diagSensitivity))) {
			...
			// arrow - right/up
				...
				canvas.moveTo(touchStartX, touchStartY);
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize * 0.75),
					touchStartY - cwh(arrowMainSize * 0.75)
				);
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize * 0.75 - arrowPointSize * 1.5),
					touchStartY - cwh(arrowMainSize * 0.75)
				);
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize * 0.75),
				...
				canvas.lineTo(
					touchStartX + cwh(arrowMainSize * 0.75),
					touchStartY - cwh(arrowMainSize * 0.75 - arrowPointSize * 1.5)
				);
				canvas.lineWidth = cwh(cursorLineSize);
				canvas.strokeStyle = arrowColor;
				canvas.stroke();

var Path = {
	...
	init: function (options) {
		...
		var outfitting = $('<div>')
			.attr({ id: 'outfitting', 'data-legend': _('supplies:') })
			.appendTo(this.panel);
	...
	updateOutfitting: function () {
		...
		// Add the armour row
		var armour = _('none');
		if ($SM.get('stores["s armour"]', true) > 0) armour = _('steel');
		else if ($SM.get('stores["i armour"]', true) > 0) armour = _('iron');
		else if ($SM.get('stores["l armour"]', true) > 0) armour = _('leather');
		var aRow = $('#armourRow');
		if (aRow.length === 0) {
	...
	updateBagSpace: function (currentBagCapacity) {
		// Update bagspace
		$('#bagspace').text(
			_('free {0}/{1}', Math.floor(Path.getCapacity() - currentBagCapacity), Path.getCapacity())
		);
	...
	createOutfittingRow: function (key, num, store) {
		...
		var row = $('<div>')
			.attr('id', 'outfit_row_' + key.replace(' ', '-'))
			.addClass('outfitRow')
			.attr('key', key);
	...
	increaseSupply: function (btn) {
		...
		if (
			Path.getFreeSpace() >= Path.getWeight(supply) &&
			cur < $SM.get('stores["' + supply + '"]', true)
		) {
	...
	handleStateUpdates: function (e) {
		if (
			...
			e.stateName.indexOf('character.perks') === 0 &&
	...
	scrollSidebar: function (direction, reset) {
		...
		if (direction == 'up') momentum = momentum * -1;

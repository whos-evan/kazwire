...
Events.Outside = [
	{
		/* Ruined traps */ title: _('A Ruined Trap'),
		...
		scenes: {
			...
			nothing: {
				text: [_('the tracks disappear after just a few minutes.'), _('the forest is silent.')],
	...
	},
	{
		...
		scenes: {
			...
			healed: {
				text: [_('the sickness is cured in time.')],
			...
			death: {
				...
				onLoad: function () {
					var numKilled =
						Math.floor(Math.random() * Math.floor($SM.get('game.population', true) / 2)) + 1;
	...
	{
		/* Beast attack */ title: _('A Beast Attack'),
		isAvailable: function () {
...

	{
		...
		isAvailable: function () {
			return (
				Engine.activeModule == Outside &&
				$SM.get('game.population', true) > 0 &&
				$SM.get('game.cityCleared')
			);
		},
		scenes: {
			start: {

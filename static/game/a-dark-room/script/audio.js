...
var AudioEngine = {
	...
	_playBackgroundMusic: function (buffer) {
		...
		if (
			...
			AudioEngine._currentBackgroundMusic.envelope.gain.cancelScheduledValues(
				AudioEngine._audioContext.currentTime
			);
			AudioEngine._currentBackgroundMusic.envelope.gain.setValueAtTime(
				currentBackgroundGainValue,
				AudioEngine._audioContext.currentTime
			);
	...
	_stopEventMusic: function () {
		...
		if (
			...
			AudioEngine._currentEventAudio.envelope.gain.cancelScheduledValues(
				AudioEngine._audioContext.currentTime
			);
			AudioEngine._currentEventAudio.envelope.gain.setValueAtTime(
				currentEventGainValue,
				AudioEngine._audioContext.currentTime
			);
	...
	playBackgroundMusic: function (src) {
		...
		AudioEngine.loadAudioFile(src).then(function (buffer) {
		...
		if (AudioEngine.AUDIO_BUFFER_CACHE[src]) {
			...
			return fetch(request)
				...
				.then(function (buffer) {
					...
					var decodeAudioDataPromise = AudioEngine._audioContext.decodeAudioData(
						buffer,
	...
	setMasterVolume: function (volume, s) {
		...
		AudioEngine._master.gain.setValueAtTime(
			currentGainValue,
			AudioEngine._audioContext.currentTime
		);

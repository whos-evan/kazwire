...
var dom = {
	...
	create: (nodeName, props = {}, content = null) => {
		...
		// Create the node
		if (dom.SVG_ELEMENTS.indexOf(nodeName) === -1) node = document.createElement(nodeName);
		else node = document.createElementNS(dom.SVG_NAMESPACE, nodeName);

		// Set class or attributes
		if (props.constructor === String) node.setAttribute('class', props);
		else for (let propName in props) node.setAttribute(propName, props[propName]);

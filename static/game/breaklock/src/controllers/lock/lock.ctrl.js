...
class LockCtrl {
	...
	mouseUpdate(t) {
		...
		let e = t.currentTarget.getBoundingClientRect(),
			x = Math.max(
				0,
				Math.min(
					PatternSVG.prototype.SVG_WIDTH,
					Math.round((PatternSVG.prototype.SVG_WIDTH / e.width) * (t.pageX - e.left))
				)
			),
			y = Math.max(
				0,
				Math.min(
					PatternSVG.prototype.SVG_WIDTH,
					Math.round((PatternSVG.prototype.SVG_WIDTH / e.height) * (t.pageY - e.top))
				)
			);
		...
		let e = t.currentTarget.getBoundingClientRect(),
			x = Math.max(
				...
				Math.min(
					PatternSVG.prototype.SVG_WIDTH,
					Math.round(
						(PatternSVG.prototype.SVG_WIDTH / e.width) * (t.targetTouches[0].pageX - e.left)
					)
				)
			),
			y = Math.max(
				...
				Math.min(
					PatternSVG.prototype.SVG_WIDTH,
					Math.round(
						(PatternSVG.prototype.SVG_WIDTH / e.height) * (t.targetTouches[0].pageY - e.top)
					)
				)
			);
		this.updatePoint(x, y);
	...
	updatePoint(x, y) {
		...
		for (let i = 0; i < 3; i++) {
			let rangeStart =
					PatternSVG.prototype.GRID_GUTTER * i +
					PatternSVG.prototype.SVG_MARGIN -
					PatternSVG.prototype.DOT_MAGNET,
				rangeEnd =
					PatternSVG.prototype.GRID_GUTTER * i +
					PatternSVG.prototype.SVG_MARGIN +
					PatternSVG.prototype.DOT_MAGNET;
		...
		if (!isEndOfPattern) this.updateLine(x, y);
	...
	triggerDot(dotIndex) {
		...
		if (this.pattern.gotDot(dotIndex)) return;
		...
		newDots.forEach((dot, index) => {
			let dotX = PatternSVG.prototype.GRID_GUTTER * (dot % 3) + PatternSVG.prototype.SVG_MARGIN,
				dotY =
					PatternSVG.prototype.GRID_GUTTER * Math.floor(dot / 3) + PatternSVG.prototype.SVG_MARGIN;
	...
	reset() {
		...
		this.currentLine = null;
		for (let i = 0; i < 9; i++) this.bigDotsEl.childNodes[i].classList.remove('active');

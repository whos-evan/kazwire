...
class GameCtrl {
	...
	newAttempt(pattern) {
		...
		if (this.isEnded) {
			...
			// Success case
			if (this.type === config.GAME.TYPE.COUNTDOWN) this.statusBar.stopCountdown();
	...
	buildPatternSVG(pattern, match) {
		...
		if (match) PatternSVG.prototype.addCombinaison.apply(attemptSVG, match);

		let svgPattern = attemptSVG.getSVG();

		// Add the success class to the SVG
		if (match[0] === pattern.dotLength) svgPattern.classList.add('success');

...
class SummaryCtrl {
	...
	setupTemplate() {
		...
		for (let action in config.GAME.ACTIONS) {
			let btn = dom.create(
				...
				[dom.icon(action.toLowerCase()), dom.create('span', {}, action)]
			);
		...
		let feedbackEl = dom.create('div', 'summary-feedback bloc', [
			dom.create('p', {}, [
				...
				dom.create(
					'a',
					{ href: config.SOCIAL.PLATFORMS.TWITTER.URL('', '@mxwllt', ['breaklock']) },
					'@mxwllt'
				)
			])
		]);
	...
	init() {
		this.actionButtons.forEach((btn) =>
			btn.addEventListener('click', this.triggerAction.bind(this))
		);
	}
	...
	updateSocialLinks() {
		this.socialButtons.forEach((item) => {
			...
			item.setAttribute(
				'href',
				socialObj.URL(config.URL, config.SOCIAL.MESSAGE, config.SOCIAL.TAGS)
			);
		});
	}
}

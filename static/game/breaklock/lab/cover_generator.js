...
class PatternSVG {
	constructor() {
		...
		this.el.setAttribute(
			'viewBox',
			'0 0 ' + this.GRID_GUTTER * 3 * width + ' ' + this.GRID_GUTTER * 3 * height
		);
	}
	...
	addPattern(pattern, size = 14, color = '#fff', x, y) {
		...
		for (let i = 1; i < pattern.suite.length; i++) {
			...
			line.setAttribute(
				'x1',
				x * 3 * this.GRID_GUTTER + (pattern.suite[i - 1] % 3) * this.GRID_GUTTER
			);
			line.setAttribute(
				'y1',
				y * 3 * this.GRID_GUTTER + Math.floor(pattern.suite[i - 1] / 3) * this.GRID_GUTTER
			);
			line.setAttribute('x2', x * 3 * this.GRID_GUTTER + (pattern.suite[i] % 3) * this.GRID_GUTTER);
			line.setAttribute(
				'y2',
				y * 3 * this.GRID_GUTTER + Math.floor(pattern.suite[i] / 3) * this.GRID_GUTTER
			);
		...
		lastDot.setAttribute(
			'cy',
			y * 3 * this.GRID_GUTTER + Math.floor(lastDotIndex / 3) * this.GRID_GUTTER
		);

...
M.launch = function () {
	...
	M.init = function (div) {
		...
		M.goodTooltip = function (id) {
			return function () {
				...
				var str =
					...
					'<div class="name">' +
					me.name +
					' <span style="font-size:12px;opacity:0.8;">(from <span style="font-variant:small-caps;">' +
					me.company +
					'</span>)</span> <span class="bankSymbol">' +
					me.symbol +
					' <span class="bankSymbolNum' +
					(delta >= 0 ? ' bankSymbolUp' : delta < 0 ? ' bankSymbolDown' : '') +
					'">' +
					(delta +
						'' +
						(delta == Math.floor(delta) ? '.00' : delta * 10 == Math.floor(delta * 10) ? '0' : '') +
						'%') +
					'</span></span></div>' +
					...
					me.desc +
					'</q>' +
					'<div class="line"></div><div style="font-size:11px;">&bull; <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-icon[0] * 48 +
					'px ' +
					-icon[1] * 48 +
					'px;"></div> ' +
					...
					': currently worth <b>$' +
					Beautify(val, 2) +
					'</b> per unit.<br>&bull; You currently own <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-icon[0] * 48 +
					'px ' +
					-icon[1] * 48 +
					'px;"></div> <b>' +
					...
					me.name +
					' (worth <b>$' +
					Beautify(val * me.stock, 2) +
					'</b>).<br>&bull; Your warehouses can store up to <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-icon[0] * 48 +
					'px ' +
					-icon[1] * 48 +
					'px;"></div> <b>' +
					Beautify(M.getGoodMaxStock(me)) +
					'</b>x ' +
					me.name +
					'.<br>&bull; You may increase your storage space by upgrading your offices and by buying more <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-me.building.iconColumn * 48 +
					'px ' +
					0 * 48 +
					'px;"></div> ' +
					me.building.plural +
					'. You also get 10 extra storage space per ' +
					me.building.single +
					' level (currently: <b>+' +
					me.building.level * 10 +
					'</b>).<br>&bull; The average worth of this stock and how high it can peak depends on the building it is tied to, along with the level of your <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-15 * 48 +
					'px ' +
					0 * 48 +
					'px;"></div> banks.</div>' +
		...
		M.tradeTooltip = function (id, n) {
			return function () {
				...
				var str =
					'<div style="padding:8px 4px;min-width:128px;text-align:center;font-size:11px;">' +
					'<div style="font-size:9px;opacity:0.6;">Stock: <b' +
					(!buyOrSell && stock == 0 ? ' class="red"' : '') +
					'>' +
					Beautify(stock) +
					'</b>/<b' +
					(buyOrSell && stock >= maxStock ? ' class="red"' : '') +
					'>' +
					Beautify(maxStock) +
					'</b></div>' +
					'<div class="line"></div>' +
					...
					' <b>' +
					Beautify(n) +
					'</b>x <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
					-me.icon[0] * 48 +
					'px ' +
					-me.icon[1] * 48 +
					'px;"></div> ' +
					...
					'</b> each</div>' +
					(overhead > 1
						? '<div style="font-size:9px;opacity:0.6;">(+<b>' +
						  Beautify((overhead - 1) * 100, 2) +
						  '%</b> overhead)</div>'
						: '') +
					'<div class="line"></div>' +
					'<div style="font-size:9px;opacity:0.6;font-weight:bold;">' +
					(buyOrSell ? 'you spend' : 'you earn') +
					':</div>' +
					'<div><b class="hasTinyCookie ' +
					(n <= 0 ? 'gray' : Game.cookies >= cost * n || !buyOrSell ? 'green' : 'red') +
					'">' +
					Beautify(cost * n) +
					'</b></div>' +
					(n > 0
						? '<div style="font-size:9px;opacity:0.6;font-weight:bold;">($' +
						  ...
						  ')</div>' +
						  '<div style="font-size:9px;opacity:0.6;font-weight:bold;">(' +
						  Game.sayTime(val * overhead * n * Game.fps, -1) +
						  ' of CpS)</div>'
						: '') +
					((me.last == 1 && !buyOrSell) || (me.last == 2 && buyOrSell)
						? '<div class="line"></div><div class="red">You cannot buy and sell this stock in the same tick.</div>'
						: '') +
					'</div>';
				return str;
			};
		...
		M.getGoodMaxStock = function (good) {
			...
			return Math.ceil(
				good.building.highest * (M.officeLevel > 4 ? 1.5 : 1) + bonus + good.building.level * 10
			);
		};
		...
		M.buyGood = function (id, n) {
			...
			if (
				...
				me.last != 2 &&
				Game.cookies >= cost * n &&
				me.stock + n <= M.getGoodMaxStock(me)
			) {
		...
		M.offices = [
			{
				...
				icon: [0, 33],
				cost: [100, 2],
				desc: 'This is your starting office.<br>Upgrading will grant you:<br><b><!--&bull; +1 opportunity slot<br>-->&bull; +25 warehouse space for all goods</b>'
			},
			{
				name: 'Tiny bank',
				icon: [9, 33],
				cost: [200, 4],
				desc: 'This is your office.<br>Upgrading will grant you:<br><b>&bull; +1 loan slot<br>&bull; +50 warehouse space for all goods</b>'
			},
			{
				name: 'Loaning company',
				icon: [10, 33],
				cost: [350, 8],
				desc: 'This is your office.<br>Upgrading will grant you:<br><!--<b>&bull; +1 opportunity slot<br>-->&bull; +75 warehouse space for all goods</b>'
			},
			{
				name: 'Finance headquarters',
				icon: [11, 33],
				cost: [500, 10],
				desc: 'This is your office.<br>Upgrading will grant you:<br><b>&bull; +1 loan slot<br>&bull; +100 warehouse space for all goods</b>'
			},
			{
				name: 'International exchange',
				icon: [12, 33],
				cost: [700, 12],
				desc: 'This is your office.<br>Upgrading will grant you:<br><b>&bull; +1 loan slot<br><!--&bull; +1 opportunity slot<br>-->&bull; +50% base warehouse space for all goods</b>'
			},
			{
				name: 'Palace of Greed',
				icon: [18, 33],
				cost: 0,
				desc: 'This is your office.<br>It is fully upgraded. Its lavish interiors, spanning across innumerable floors, are host to many a decadent party, owing to your nigh-unfathomable wealth.'
			}
		];
		...
		M.officeTooltip = function () {
			return function () {
				...
				var str =
					...
					'<div class="name">' +
					me.name +
					' <span style="font-size:11px;opacity:0.6;">[Level ' +
					(M.officeLevel + 1) +
					' offices]</span></div>' +
					...
					'</div>' +
					(me.cost
						? '<div class="line"></div><div style="font-size:11px;padding-left:24px;position:relative;">' +
						  '<div id="bankOfficeIcon" class="icon" style="position:absolute;left:0px;top:6px;pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
						  -11 * 48 +
						  'px ' +
						  -0 * 48 +
						  'px;"></div>' +
						  'Upgrading will cost you <b class="' +
						  (Game.Objects['Cursor'].amount >= me.cost[0] ? 'green' : 'red') +
						  '">' +
						  me.cost[0] +
						  ' cursors</b>.<br>' +
						  'Upgrading requires <b class="' +
						  (Game.Objects['Cursor'].level >= me.cost[1] ? 'green' : 'red') +
						  '">level ' +
						  me.cost[1] +
						  ' cursors</b>.' +
						  '</div>'
		...
		M.getMaxBrokers = function () {
			return Math.ceil(Game.Objects['Grandma'].highest / 10 + Game.Objects['Grandma'].level);
		};
		M.getBrokerPrice = function () {
			return Game.cookiesPsRawHighest * 60 * 20;
		};
		M.brokersTooltip = function () {
			return function () {
				...
				var str =
					...
					-icon[1] * 48 +
					'px;"></div>' +
					'<div class="name">Stockbrokers <span style="font-size:11px;opacity:0.6;">(you have ' +
					Beautify(M.brokers) +
					')</span></div>' +
					'<div class="line"></div><div class="description" style="font-size:11px;">' +
					...
					'&bull; Current overhead costs thanks to your ' +
					Beautify(M.brokers) +
					' broker' +
					(M.brokers == 1 ? '' : 's') +
					': <b>+' +
					Beautify(20 * Math.pow(0.95, M.brokers), 2) +
					'%</b><br>' +
					'&bull; Buying a broker costs <b class="hasTinyCookie ' +
					(Game.cookies >= M.getBrokerPrice() ? 'green' : 'red') +
					'">20 minutes</b> of CpS (that\'s $1200).<br>' +
					'&bull; Maximum number of brokers you can own: <b class="' +
					(M.brokers < M.getMaxBrokers() ? 'green' : 'red') +
					'">' +
					Beautify(M.getMaxBrokers()) +
					"</b> (the highest amount of grandmas you've owned this run, divided by 10, plus your grandma level)<br>" +
					...
					'Hiring a new broker will cost you <b class="hasTinyCookie ' +
					(Game.cookies >= M.getBrokerPrice() ? 'green' : 'red') +
					'">' +
					Beautify(M.getBrokerPrice()) +
					' cookies</b>.' +
					'</div>' +
					'</div>' +
					'</div>';
		...
		M.loanTooltip = function (id) {
			return function () {
				...
				var str =
					'<div style="padding:8px 4px;min-width:350px;">' +
					'<div class="name">Take out ' +
					loan[0] +
					'</div>' +
					'<div class="line"></div><div class="description" style="font-size:11px;">' +
					'By taking this loan, you will get <b class="green">+' +
					Math.round((loan[1] - 1) * 100) +
					'%</b> CpS for the next <b>' +
					Game.sayTime(60 * loan[2] * Game.fps) +
					'</b>.<br>' +
					'However, you will get <b class="red">' +
					Math.round((loan[3] - 1) * 100) +
					'%</b> CpS for the next <b>' +
					Game.sayTime(60 * loan[4] * Game.fps) +
					'</b> after that.<br>' +
					'You must also pay an immediate downpayment of <b class="hasTinyCookie red">' +
					Beautify(Game.cookies * loan[5]) +
					'</b> (<b>' +
					loan[5] * 100 +
					'%</b> of your current bank).<br>' +
					'<q>' +
					loan[6] +
					'</q>' +
					'</div>';
		...
		M.refillTooltip = function () {
			return (
				'<div style="padding:8px;width:300px;font-size:11px;text-align:center;">Click to refill your opportunity timer (and give a quick burst to your economy) for <span class="price lump">1 sugar lump</span>.' +
				(Game.canRefillLump()
		...
		str +=
			'<div id="bankHeader" style="z-index:10;position:relative;">' +
			'<div>' +
			...
			'<div id="bankOffice" style="display:inline-block;padding:0px 4px;" ' +
			Game.getDynamicTooltip(
				'Game.ObjectsById[' + M.parent.id + '].minigame.officeTooltip()',
				'this'
			) +
			'><div id="bankOfficeIcon" class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -12px -14px;vertical-align:middle;background-position:' +
			-0 * 48 +
			'px ' +
			-33 * 48 +
			'px;"></div><span id="bankOfficeName" class="bankSymbol" style="width:128px;"></span><div class="bankButton bankButtonBuy bankButtonOff" id="bankOfficeUpgrade">-</div></div>' +
			'<div id="bankBrokers" style="display:inline-block;padding:0px 4px;" ' +
			Game.getDynamicTooltip(
				'Game.ObjectsById[' + M.parent.id + '].minigame.brokersTooltip()',
				'this'
			) +
			'><div id="bankBrokersIcon" class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -12px -14px;vertical-align:middle;background-position:' +
			-1 * 48 +
			'px ' +
			-33 * 48 +
			'px;"></div><span id="bankBrokersText" class="bankSymbol" style="width:96px;">no brokers</span><div class="bankButton bankButtonBuy bankButtonOff" id="bankBrokersBuy">Hire</div></div>' +
			'<div style="display:inline-block;padding:0px 4px;"><div id="bankLoan1" style="display:none;" class="bankButton bankButtonSell bankButtonOff" ' +
			Game.getDynamicTooltip(
				'Game.ObjectsById[' + M.parent.id + '].minigame.loanTooltip(1)',
				'this'
			) +
			'>1st loan</div><div id="bankLoan2" style="display:none;" class="bankButton bankButtonSell bankButtonOff" ' +
			Game.getDynamicTooltip(
				'Game.ObjectsById[' + M.parent.id + '].minigame.loanTooltip(2)',
				'this'
			) +
			'>2nd loan</div><div id="bankLoan3" style="display:none;" class="bankButton bankButtonSell bankButtonOff" ' +
			Game.getDynamicTooltip(
				'Game.ObjectsById[' + M.parent.id + '].minigame.loanTooltip(3)',
				'this'
			) +
			'>3rd loan</div></div>' +
		...
		for (var i = 0; i < M.goodsById.length; i++) {
			...
			str +=
				'<div class="bankGood" id="bankGood-' +
				me.id +
				'">' +
				'<div ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.goodTooltip(' + me.id + ')',
					'this'
				) +
				'>' +
				'<div class="icon" style="z-index:20;pointer-events:none;position:absolute;left:0px;top:0px;transform:scale(0.5);margin:-16px -16px;background-position:' +
				-me.icon[0] * 48 +
				'px ' +
				-me.icon[1] * 48 +
				'px;"></div>' +
				'<div class="bankSymbol" style="margin:1px 0px;display:block;padding:2px 0px;width:100%;overflow:hidden;white-space:nowrap;">' +
				me.symbol +
				' <span id="bankGood-' +
				me.id +
				'-sym" class="bankSymbolNum">-.--%</span></div>' +
				'<div class="bankViewHide bankSimpleButton" id="bankGood-' +
				me.id +
				'-viewHide">Hide</div>' +
				'<div class="bankSymbol" style="margin:1px 0px;display:block;font-size:10px;width:100%;background:linear-gradient(to right,transparent,#333,#333,transparent);padding:2px 0px;overflow:hidden;white-space:nowrap;">value: <span style="font-weight:bold;color:#fff;" id="bankGood-' +
				me.id +
				'-val">-</span></div>' +
				'<div class="bankSymbol" style="margin:1px 0px;display:block;font-size:10px;width:100%;background:linear-gradient(to right,transparent,#333,#333,transparent);padding:2px 0px;overflow:hidden;white-space:nowrap;" id="bankGood-' +
				me.id +
				'-stockBox">stock: <span style="font-weight:bold;" id="bankGood-' +
				me.id +
				'-stock">-</span><span style="font-weight:bold;" id="bankGood-' +
				me.id +
				'-stockMax">/-</span></div>' +
				'</div>' +
				'<div style="position:relative;white-space:nowrap;">' +
				'<div style="padding:3px 2px;width:22px;" class="bankSymbol">Buy</div>' +
				'<div class="bankButton bankButtonBuy" id="bankGood-' +
				me.id +
				'_1" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',1)',
					'this'
				) +
				'>1</div>' +
				'<div class="bankButton bankButtonBuy" id="bankGood-' +
				...
				'_10" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',10)',
					'this'
				) +
				'>10</div>' +
				'<div class="bankButton bankButtonBuy" id="bankGood-' +
				...
				'_100" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',100)',
					'this'
				) +
				'>100</div>' +
				'<div style="width:28px;" class="bankButton bankButtonBuy" id="bankGood-' +
				me.id +
				'_Max" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',10000)',
					'this'
				) +
				'>Max</div>' +
				'<br>' +
				'<div style="padding:3px 2px;width:22px;" class="bankSymbol">Sell</div>' +
				'<div class="bankButton bankButtonSell" id="bankGood-' +
				me.id +
				'_-1" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-1)',
				...
				'>100</div>' +
				'<div style="width:28px;" class="bankButton bankButtonSell" id="bankGood-' +
				me.id +
				'_-All" ' +
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-10000)',
		...
		var str = '';
		str +=
			'<div style="position:absolute;left:2px;top:2px;z-index:10;">' +
			...
			(Game.sesame
				? '<div id="bankCheatSpeed" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">Toggle speed</div>'
				: '') +
			'</div>' +
		...
		for (var i = 0; i < M.goodsById.length; i++) {
			...
			str +=
				'<div id="bankGood-' +
				me.id +
				'-graphIcon" class="icon bankGraphIcon" style="pointer-events:none;transform:scale(0.5);background-position:' +
				-me.icon[0] * 48 +
				'px ' +
				-me.icon[1] * 48 +
				'px;"></div>';
		...
		AddEvent(l('bankOfficeUpgrade'), 'click', function (e) {
			...
			if (
				me.cost &&
				Game.Objects['Cursor'].amount >= me.cost[0] &&
				Game.Objects['Cursor'].level >= me.cost[1]
			) {
		...
		AddEvent(M.graph, 'mousemove', function (e) {
			...
			bankGraphMouseDetect: for (var i = M.goodsById.length - 1; i >= 0; i--) {
				var id = i;
				var me = M.goodsById[id];
				...
				for (var iR = 0; iR < rows; iR++) {
					if (me.vals.length >= 2 + iR) {
						...
						if (
							x >= width - span * iR - span - 2 &&
							x <= width - span * iR + 2 &&
							y >= height - min * M.graphScale - 6 &&
							y <= height - min * M.graphScale + Math.max(3, max * M.graphScale) + 6
						) {
							isOnLine = i;
							Game.tooltip.draw(
								0,
								'<div style="width:128px;font-size:10px;text-align:center;"><div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' +
									-me.icon[0] * 48 +
									'px ' +
									-me.icon[1] * 48 +
									'px;"></div> <b>' +
									me.name +
									'</b><br>valued at <b>$' +
									Beautify(me.vals[0 + iR], 2) +
									'</b><br>' +
									Game.sayTime((iR + 1) * M.secondsPerTick * Game.fps) +
									' ago</div>',
								'top'
							);
	...
	M.load = function (str) {
		...
		M.graphCols = parseInt(spl2[i2++] || M.graphCols);
		M.setCols();
		M.tickT = 0;
	...
	M.tick = function () {
		for (var i = 0; i < M.goodsById.length; i++) {
			...
			if (me.mode == 0) {
				me.d *= 0.95;
				me.d += 0.05 * (Math.random() - 0.5);
			} else if (me.mode == 1) {
				me.d *= 0.99;
				me.d += 0.05 * (Math.random() - 0.1);
			} else if (me.mode == 2) {
				me.d *= 0.99;
				me.d -= 0.05 * (Math.random() - 0.1);
			} else if (me.mode == 3) {
				me.d += 0.15 * (Math.random() - 0.1);
				me.val += Math.random();
			} else if (me.mode == 4) {
				me.d -= 0.15 * (Math.random() - 0.1);
				me.val -= Math.random();
			...
			if (me.mode == 3 && Math.random() < 0.3) {
				me.d += (Math.random() - 0.5) * 0.1;
				me.val += (Math.random() - 0.7) * 10;
			}
			if (me.mode == 3 && Math.random() < 0.03) {
				me.mode = 4;
			}
			if (me.mode == 4 && Math.random() < 0.3) {
				me.d += (Math.random() - 0.5) * 0.1;
				me.val += (Math.random() - 0.3) * 10;
			}
	...
	M.logic = function () {
		...
		if (Game.T % 10 == 0) {
			...
			for (var i = 0; i < M.goodsById.length; i++) {
				...
				if (!me.active && me.building.highest > 0) {
					me.active = true;
					me.hidden = false;
					M.toRedraw = 2;
					if (me.l) {
						M.updateGoodStyle(me.id);
						doResize = true;
					}
				}
			}
	...
	M.drawGraph = function (full) {
		...
		*/
		...
		for (
			var i = 0;
			i < height / M.graphScale;
			i += 2 //horizontal lines (1 every 2 units)
		) {
			if (i % 10 != 0) ctx.fillStyle = M.cols.line1;
			else ctx.fillStyle = M.cols.line2;
			ctx.fillRect(width - span * rows, height - Math.floor(i * M.graphScale), span * rows, 1);
		}
		for (
			var iR = 0;
			iR < rows;
			iR++ //vertical lines (1 every 10 ticks)
		) {
			if ((iR - M.ticks) % 10 != 0) continue;
			if ((iR - M.ticks) % 60 != 0) ctx.fillStyle = M.cols.line1;
			else ctx.fillStyle = M.cols.line2;
				...
				if (me.vals.length >= 2 + iR) {
					...
					if (M.graphLines == 0) {
						...
						if (M.hoverOnGood == id) {
							...
							ctx.fillRect(
								width - span * iR - span - 1,
								Math.floor(height - min * M.graphScale) - 1,
								span + 1,
								Math.max(3, Math.ceil(max * M.graphScale)) + 2
							);
						}
						ctx.fillStyle = delta > 0 ? M.cols.high : M.cols.low;
						ctx.fillRect(
							width - span * iR - span,
							Math.floor(height - min * M.graphScale),
							span - 1,
							Math.max(3, Math.ceil(max * M.graphScale))
						);
						ctx.fillRect(
							width - span * iR - span / 2 - 1,
							Math.floor(height - (min + min2) * M.graphScale),
							1,
							Math.max(3, Math.ceil((max + max2) * M.graphScale))
						);
					} else {
						if (M.hoverOnGood == id) {
							ctx.lineWidth = 4;
							...
							ctx.moveTo(
								width - span * iR - span - 1,
								Math.floor(height - me.vals[1 + iR] * M.graphScale) + 0.5
							);
							ctx.lineTo(
								width - span * iR - 1,
								Math.floor(height - me.vals[0 + iR] * M.graphScale) + 0.5
							);
							ctx.stroke();
							ctx.lineWidth = 2;
						}
	...
	M.draw = function () {
		...
		if (Game.drawT % 2 == 0 && M.toRedraw > 0 && M.graph && M.graphCtx) {
			...
			for (var i = 0; i < M.goodsById.length; i++) {
				...
				me.symbolNumL.innerHTML =
					val +
					'' +
					(val == Math.floor(val) ? '.00' : val * 10 == Math.floor(val * 10) ? '0' : '') +
					'%' /*+', '+['stable','slow rise','slow fall','fast rise','fast fall','chaotic'][me.mode]*/;
				if (val >= 0) {
					me.symbolNumL.classList.add('bankSymbolUp');
					me.symbolNumL.classList.remove('bankSymbolDown');
				} else if (val < 0) {
					me.symbolNumL.classList.remove('bankSymbolUp');
					me.symbolNumL.classList.add('bankSymbolDown');
				} else {
					me.symbolNumL.classList.remove('bankSymbolUp');
					me.symbolNumL.classList.remove('bankSymbolDown');
				}
				...
				me.graphIconL.style.transform =
					'translate(-8px,' +
					Math.floor(M.graph.height - me.vals[0] * M.graphScale) +
					'px) scale(0.5)';
			}
			M.toRedraw = 0;
		}
		if (Game.drawT % 10 == 0) {
			...
			l('bankOfficeIcon').style.backgroundPosition =
				-office.icon[0] * 48 + 'px ' + -office.icon[1] * 48 + 'px';
			...
			l('bankBrokersText').innerHTML =
				M.brokers == 0 ? 'no brokers' : M.brokers == 1 ? '1 broker' : M.brokers + ' brokers';
			if (M.brokers < M.getMaxBrokers() && Game.cookies >= M.getBrokerPrice())
				l('bankBrokersBuy').classList.remove('bankButtonOff');
			...
			for (var id = 1; id < 4; id++) {
				if (Game.hasBuff('Loan ' + id) || Game.hasBuff('Loan ' + id + ' (interest)'))
					l('bankLoan' + id).classList.add('bankButtonOff');
			...
			if (M.profit > 0) {
				...
				it.classList.remove('bankSymbolDown');
			} else if (M.profit < 0) {
				it.classList.add('bankSymbolDown');
				it.classList.remove('bankSymbolUp');
			}

			l('bankNextTick').innerHTML =
				'Next tick in ' + Game.sayTime(Game.fps * M.secondsPerTick - M.tickT + 30, -1) + '.';
		}
	};

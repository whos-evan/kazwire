...
var LaunchDungeons = function () {
	...
	new Game.DungeonType('Factory').nameGenerator = function () {
		var str = '';
		str +=
			Game.GetWord(choose(['secret', 'ruined', 'magical'])) +
			' ' +
			choose([
				'factory',
				'factories',
	...
	new Game.DungeonType('Mine').nameGenerator = function () {
		...
		str +=
			Game.GetWord(choose(['secret', 'ruined', 'magical'])) +
			' ' +
			choose(['chocolate', 'chocolate', 'chocolate', 'white chocolate', 'sugar', 'cacao']) +
			' ' +
			choose([
				'mine',
				'mines',
	...
	new Game.DungeonType('Portal').nameGenerator = function () {
		...
		str +=
			Game.GetWord(choose(['secret', 'ruined', 'magical'])) +
			' ' +
			choose(['portal', 'gate', 'dimension', 'warpgate', 'door']);
		return str;
	};
	...
	*/
	...
	new Game.Monster(
		...
		[0, 0],
		1,
		{ hp: 5, might: 2, guard: 2, speed: 6, dodge: 6, rarity: 0.7 },
		basicLoot
	);
	new Game.Monster(
		'Elder doughling',
	...
	new Game.Monster(
		'Angry sentient cookie',
		'angrySentientCookie',
		[5, 0],
		5,
		{ hp: 16, might: 8, guard: 4, speed: 5, dodge: 5, rarity: 1 },
		basicLoot
	);
	new Game.Monster(
		'Baby sentient cookie',
		'babySentientCookie',
		[4, 0],
		1,
		{ hp: 3, might: 1, guard: 1, speed: 7, dodge: 7, rarity: 1 },
		basicLoot
	);
	new Game.Monster(
		'Burnt sentient cookie',
		'burntSentientCookie',
		[6, 0],
		5,
		{ hp: 16, might: 12, guard: 2, speed: 3, dodge: 2, rarity: 0.2 },
		basicLoot
	);
	new Game.Monster(
		'Raw sentient cookie',
		...
		[5, 0],
		5,
		{ hp: 16, might: 6, guard: 4, speed: 7, dodge: 7, rarity: 0.2 },
		basicLoot
	);
	new Game.Monster(
		'Sugar bunny',
		...
		[8, 0],
		5,
		{ hp: 10, might: 3, guard: 8, speed: 12, dodge: 9, rarity: 0.001 },
	...
	new Game.Monster(
		...
		[0, 2],
		6,
		{ hp: 18, might: 6, guard: 8, speed: 3, dodge: 2, rarity: 0.5 },
		goodLoot
	);
	new Game.Monster(
		'Crazed chip-spurter',
		...
		[0, 2],
		6,
		{ hp: 15, might: 6, guard: 8, speed: 5, dodge: 3, rarity: 0.5 },
		goodLoot
	);
	new Game.Monster(
		'Alarm bot',
		...
		[3, 2],
		2,
		{ hp: 6, might: 3, guard: 5, speed: 8, dodge: 8, rarity: 0.5 },
		basicLoot
	);
	new Game.Monster(
		'Chirpy',
		...
		[4, 2],
		3,
		{ hp: 7, might: 4, guard: 6, speed: 9, dodge: 9, rarity: 0.01 },
	...
	new Game.Monster(
		...
		[1, 2],
		4,
		{ hp: 14, might: 5, guard: 5, speed: 6, dodge: 4, rarity: 0.6 },
		basicLoot
	);
	new Game.Monster(
		'Disgruntled overseer',
		'disgruntledOverseer',
		[1, 2],
		7,
		{ hp: 22, might: 7, guard: 5, speed: 6, dodge: 4, rarity: 0.5 },
		basicLoot
	);
	new Game.Monster(
		'Disgruntled cleaning lady',
		'disgruntledCleaningLady',
		[2, 2],
		4,
		{ hp: 13, might: 4, guard: 5, speed: 7, dodge: 6, rarity: 0.3 },
		basicLoot
	);
	...
	new Game.Monster(
		...
		[0, 3],
		0,
		{ hp: 60, might: 14, guard: 12, speed: 4, dodge: 0, rarity: 1 },
		bossLoot
	); //boss
	...
	new Game.Monster(
		...
		[1, 3],
		0,
		{ hp: 60, might: 12, guard: 14, speed: 4, dodge: 0, rarity: 0.7 },
		bossLoot
	); //boss
	...
	Game.Entity = function (
		...
		dungeon,
		pic,
		stats //objects you could find on the map : doors, mobs, interactables, items, player, exits...
	) {
		this.type = type;
		...
		this.Say = function (what) {
			if (this.type == 'monster') {
				if (Game.Monsters[this.subtype].quotes[what])
					this.dungeon.Log(
						this.subtype +
							' : "<span style="color:#f96;">' +
							choose(Game.Monsters[this.subtype].quotes[what].split('|')) +
							'</span>"'
					);
			}
		};
		...
		this.GoTo = function (
			x,
			y //AI to move to a specific point
		) {
			this.targets = [];
			...
			if (this.y > y) this.targets.push([0, -1]);
			if (!this.Move()) {
				//really stuck? try to maneuver laterally!
				this.targets = [];
		...
		this.Flee = function (
			x,
			y //AI to run away from a specific point
		) {
			this.targets = [];
			if (this.x > x) this.targets.push([1, 0]);
			if (this.x < x) this.targets.push([-1, 0]);
			...
			if (this.targets.length > 0) {
				...
				if (this.type == 'hero') goodTargets = this.targets;
					...
					for (var i in this.targets) {
						...
						if (this.dungeon.CheckObstacle(this.x + thisTarget[0], this.y + thisTarget[1]) != -1)
							goodTargets.push([thisTarget[0], thisTarget[1]]);
					}
				}
		...
		this.HitBy = function (
			...
			if (this.type == 'destructible' && by.type == 'hero') {
				...
				(this.type == 'monster' && by.type == 'hero') ||
				(this.type == 'hero' && by.type == 'monster' && this.stats.hp > 0)
			) {
				//it's a fight!
				by.stuck = 0;
				...
				var damage = Math.round(
					Math.max(
						...
						Math.min(
							by.stats.might,
							Math.pow((by.stats.might + 2.5) / Math.max(1, this.stats.guard), 2)
						)
					) *
						(0.8 + Math.random() * 0.4 + Math.pow(Math.random() * 0.8, 6))
				);
					...
					if (this.type == 'hero') {
						...
						this.dungeon.Log(
							'<span style="color:#f66;">' +
								Game.Heroes[this.subtype].name +
								' has been defeated.</span>'
						);
					...
					if (this.type == 'monster' && by.type == 'hero') {
						...
						if (Game.Monsters[this.subtype].loot) {
							...
							if (loot.cookies && (!loot.cookies.prob || Math.random() < loot.cookies.prob)) {
								...
								entity.value = Math.round(
									loot.cookies.min + Math.random() * (loot.cookies.max - loot.cookies.min)
								);
							}
						}
						if (this.onKill) this.onKill();
		...
		this.Turn = function () //do this every turn (walk around, heal up...)
		{
			...
			if (
				(this.type == 'hero' || this.type == 'monster') &&
				this.fighting == 0 &&
				this.stats.hp < this.stats.hpm
			)
				this.stats.hp++; //heal up
			if (this.type == 'hero') {
				//collect items and cookies
				...
				for (var i in entities) {
					if (entities[i].type == 'item' && entities[i].subtype == 'cookies') {
						...
						var value = Math.ceil(
							entity.value *
								Game.Objects[this.dungeon.type].amount *
								50 *
								(1 + Math.random() * (this.stats.luck / 20))
						); //temporary; scale with matching building CpS later
						if (value > 0) {
							this.dungeon.Log(
								'<span style="color:#9f9;">Found <b>' +
									Beautify(value) +
									'</b> cookie' +
									(value == 1 ? '' : 's') +
									'!</span>'
		...
		this.GetInitiative = function () {
			return randomFloor(
				(this.stats.speed / 5) * (1 / Math.max(1, this.dungeon.heroEntity.stats.speed / 5))
			);
		};
	};
	...
	Game.Dungeon = function (type, id) {
		...
		this.GetEntities = function (
			x,
			y //returns the first entity found on tile x,y
		) {
			var entities = [];
		...
		this.CheckObstacle = function (
			x,
			y //returns 0 for no obstacle; -1 for a wall; an entity if there's at least one entity on this tile
		) {
		...
		this.Generate = function () {
			...
			for (var ii in Game.BossMonsters) {
				...
				if (me.level <= depth + this.level && Math.random() < (me.stats.rarity || 1))
					monsters.push(me.name);
			}
			...
			for (
				var i = 0;
				i < Math.ceil(this.map.freeTiles.length * 0.7);
				i++ //let's fill this up with A LOT of stuff
			) {
				...
				if (tile != -1) {
					...
					if (Math.random() < 0.2) {
						//2 in 10 spawns are monsters
						var monsters = [];
						for (var ii in Game.Monsters) {
							var me = Game.Monsters[ii];
							if (
								me.level != 0 &&
								me.level <= depth + this.level &&
								Math.random() < (me.stats.rarity || 1)
							)
								monsters.push(me.name); //spawn type depending on monster level and rarity
						}
						if (monsters.length > 0) {
							this.AddEntity('monster', choose(monsters), tile[0], tile[1]);
							this.map.removeFreeTile(tile[0], tile[1]);
						}
					} //the rest of the spawns are destructibles or loot
					else {
						if (Math.random() < 0.6) {
		...
		this.Draw = function () {
			...
			str +=
				'<div id="map' +
				this.id +
				'" class="map" style="width:' +
				9 * 16 +
				'px;height:' +
				9 * 16 +
				'px;"><div class="mapContainer" id="mapcontainer' +
				this.id +
			...
			str +=
				...
				(9 * 16 + 16) +
				'px;">' +
				'<a class="control west" onclick="Game.HeroesById[' +
				this.hero.id +
				'].Move(-1,0);"></a><br>' +
				'<a class="control east" onclick="Game.HeroesById[' +
				this.hero.id +
				'].Move(1,0);"></a><br>' +
				'<a class="control north" onclick="Game.HeroesById[' +
				this.hero.id +
				'].Move(0,-1);"></a><br>' +
				'<a class="control south" onclick="Game.HeroesById[' +
				this.hero.id +
				'].Move(0,1);"></a><br>' +
				'<a class="control middle" onclick="Game.HeroesById[' +
				this.hero.id +
				'].Move(0,0);"></a><br>' +
				'</div>';
			str +=
				'<div style="position:absolute;left:' +
				(9 * 16 + 16 + 48 * 3) +
				'px;bottom:16px;height:100%;">' +
				'<div class="dungeonName"><a onclick="Game.ObjectsById[' +
				this.id +
				'].setSpecial(0);">Exit</a> - <span class="title" style="font-size:12px;">' +
				this.name +
				'</span> lvl.' +
				(this.level + 1) +
				'</div>' +
				'<div id="heroSlot' +
				this.id +
				'" class="mobSlot"><div id="picHero' +
				this.id +
				'" class="mobPic"></div><div id="nameHero' +
				this.id +
				'" class="title mobName"></div><div class="hpmBar"><div id="hpHero' +
				this.id +
				'" class="hpBar"></div></div></div>' +
				'<div id="monsterSlot' +
				this.id +
				'" class="mobSlot" style="left:128px;"><div id="picMonster' +
				this.id +
				'" class="mobPic"></div><div id="nameMonster' +
				this.id +
				'" class="title mobName"></div><div class="hpmBar"><div id="hpMonster' +
				this.id +
				'" class="hpBar"></div></div></div>' +
				'</div>' +
				'<div id="dungeonLog' +
				this.id +
				'" class="dungeonLog"></div>';
			l('rowSpecial' + this.id).innerHTML =
				'<div style="width:100%;height:100%;z-index:10000;position:absolute;left:0px;top:0px;">' +
				str +
				'</div>';
		...
		this.Turn = function () {
			...
			if (this.currentOpponent) {
				...
				l('hpMonster' + this.id).style.width =
					Math.round((this.currentOpponent.stats.hp / this.currentOpponent.stats.hpm) * 100) + '%';
				l('picMonster' + this.id).style.backgroundImage =
					'url(img/' + Game.Monsters[this.currentOpponent.subtype].pic + '.png)';
			...
			this.currentOpponent = 0;
			l('hpHero' + this.id).style.width =
				Math.round((this.heroEntity.stats.hp / this.heroEntity.stats.hpm) * 100) + '%';
		...
		this.DrawButton = function () {
			...
			str +=
				'<div style="width:144px;height:144px;position:absolute;left:0px;bottom:0px;"><a class="specialButtonPic" style="background-image:url(img/' +
				this.portalPic +
				'.png);" onclick="Game.ObjectsById[' +
				this.id +
				'].setSpecial(1);"><div class="specialButtonText">Enter dungeons</div></a></div>';
			return str;
		};
		...
		this.FailLevel = function () {
			this.Log(
				'Cookies made this run : ' +
					Beautify(this.cookiesMadeThisRun) +
					' | Monsters defeated this run : ' +
					Beautify(this.monstersKilledThisRun)
			);
			this.cookiesMadeThisRun = 0;
			this.monstersKilledThisRun = 0;
			this.level = 0;
	...
	Game.DungeonLocationChain = function (
		map,
		x,
		y //return an array of the rooms between the root room and this tile's room, inclusive
	) {
	...
	Game.DungeonLinkLocationChains = function (
		start,
		end //return the room in which the first location chain should go to to get closer to the second location chain
	) {
		/*
		4 cases
	...
	Game.Objects['Factory'].special = function () {
		...
		this.specialDrawFunction = function () {
			this.dungeon.Refresh();
		};
		this.drawSpecialButton = function () {
		...
		this.EachFrame = function () {
			if (this.dungeon.auto) {
				...
				if (this.dungeon.timer == 0) {
					this.dungeon.timer =
						Game.fps *
						(Math.max(0.1, 2 - this.dungeon.hero.stats.speed * 0.2) +
							Math.max(this.dungeon.timerWarmup, 0));
					if (this.dungeon.timerWarmup > 0) this.dungeon.timerWarmup--;

					var dungeon = this.dungeon;
					var hero = dungeon.heroEntity;

					var targetRoom = Game.DungeonLinkLocationChains(
						Game.DungeonLocationChain(dungeon.map, hero.x, hero.y),
						Game.DungeonLocationChain(dungeon.map, dungeon.map.exit[0], dungeon.map.exit[1])
					);
					var targetTile =
						targetRoom.gen == 0 || targetRoom.id == dungeon.map.getRoom(hero.x, hero.y).id
							? [dungeon.map.exit[0], dungeon.map.exit[1]]
							: targetRoom.door;
	...
	Game.Hero = function (name, pic, portrait, icon) {
		...
		this.Say = function (what) {
			if (this.dialogue[what])
				Game.Dungeons[this.inDungeon].Log(
					this.name +
						' : "<span style="color:#99f;">' +
						choose(this.dialogue[what].split('|')) +
						'</span>"'
				);
		};
	...
	hero.dialogue = {
		...
		entrance:
			"Chipping in!|Welp, here goes nothing!|I wonder what I'll find!|Hey, this place is new!|This place seems familiar.|Let's make it happen.",
		completion:
			"I'm one smart cookie.|Oh yeah!|Let's explore some more!|That was easy!|That sure was fun!|I'm not lost, am I?|More exploring? Sure, why not!",
		defeat:
			'B-better luck next time.|That really hurt!|I yield! I yield!|That went badly.|No half-baked excuses next time.|I think I scraped my knee!|Owie.|Woopsie!',
	...
	hero.dialogue = {
		intro:
			"I'm Crumb. I look like this because of a baking accident when I was little. Big deal. At least now I don't get hurt as easily as others, I guess.",
		...
		entrance:
			"Let's do this, I guess.|Well, let's go...|I gotta go in there?|Are we really doing this?|I hope I won't get lost like last time.|Let's get this over with.",
		completion:
			"I... I did it...|I'm glad that's over.|What, there's more?|In I go, I guess.|It doesn't end, does it?|But it's dark in there.",
		defeat:
			"I, uh, ouch.|Why does that always happen to me?|I'm just no good, am I?|Oh no.|I'm... I'm not crying.|Well that wasn't fun at all.|I'm sorry I failed you.|Please... make them go away...",
		'meet Ascended Baking Pod':
			"That thing shouldn't even be alive.|Is that where they all came from?",
	...
	hero.dialogue = {
		...
		entrance:
			"Alright, let's do this!|I-if I really have to.|I-in there? By myself?|...won't you come with me this time?|H-here I go!",
		completion:
			"Oh... oh my.|That's... I uh, I'm glad.|Y-yeah that was real easy. Piece of pie!|T-too easy, right?|S-so many cookies...|Ooh? F-fascinating.",
		defeat:
			"I-if you can't beat them... join them.|I-it's because I stutter, isn't it?|W-well that's just no good at all.|I, uh, I meant for that to happen.|H-how embarrassing.",
		'meet Ascended Baking Pod': "W-whoah... it's... magnificent...",
		'win against Ascended Baking Pod':
			"I'm sorry, buddy.|I... I think I hurt it...|Oh no... I-I think I broke it..."
	};
	hero.stats = {
		hp: 25,
		hpm: 25,
	...
	hero.dialogue = {
		...
		entrance:
			"Glad to be of service!|Oooh this one'll be interesting.|This will be a good one, I can feel it!|Here I come!",
		completion:
			"Over already?|Let's explore some more!|That was lucky!|That was no luck, I'm just that good.|Alright, let's move on!|I'm just getting warmed up!",
		defeat:
			"I can't believe it!|...This is a joke, right?|Hey! No fair!|B-but...|I'm gonna need a bandaid. And some hot chocolate.|I'll, uh, try again later.|Bad luck! Bad luck!",

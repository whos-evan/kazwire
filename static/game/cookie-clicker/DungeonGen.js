...
var DungeonGen = function () {
	...
	this.Map = function (w, h, seed, params) {
		...
		if (undefined != seed) this.seed = seed;
		else {
			Math.seedrandom();
	...
	this.Map.prototype.getRoom = function (x, y) {
		if (this.data[x][y][1] != -1) return this.rooms[this.data[x][y][1]];
		else return -1;
	};
	...
	this.Map.prototype.isWall = function (x, y) {
		...
		for (var i in this.freeWalls) {
			if (this.freeWalls[i][0] == x && this.freeWalls[i][1] == y) return n;
			else n++;
		}
		return -1;
	};
	this.Map.prototype.isFloor = function (x, y) {
		...
		for (var i in this.freeTiles) {
			if (this.freeTiles[i][0] == x && this.freeTiles[i][1] == y) return n;
			else n++;
		}
		return -1;
	};
	...
	this.Map.prototype.fill = function (what) {
		...
		for (var x = 0; x < this.w; x++) {
			for (var y = 0; y < this.h; y++) {
				if (func) this.data[x][y] = [what(this, x, y), -1, 0];
				else this.data[x][y] = [what, -1, 0];
			}
		}
		this.rooms = [];
	};
	...
	this.Map.prototype.getRoomTile = function (room, x, y) {
		...
		for (var i in room.tiles) {
			if (room.tiles[i].x == x && room.tiles[i].y == y) return n;
			else n++;
		}
		return -1;
	};
	...
	this.Map.prototype.getFloorTileInRoom = function (room) {
		var tiles = [];
		for (var i in room.tiles) {
			if (room.tiles[i].type == TILE_FLOOR_EDGE || room.tiles[i].type == TILE_FLOOR_CENTER)
				tiles.push(room.tiles[i]);
		}
		return choose(tiles);
	};
	...
	this.Map.prototype.setRoomTile = function (room, x, y, tile) {
		...
		if (
			...
			//(tile!=TILE_FLOOR_EDGE && tile!=TILE_FLOOR_CENTER && (oldTileType!=TILE_FLOOR_EDGE && oldTileType!=TILE_FLOOR_CENTER)) ||
			(tile == TILE_WALL ||
				tile == TILE_WALL_CORNER || //don't place a wall over an existing room
			...
			if (
				(tile == TILE_FLOOR_EDGE || tile == TILE_FLOOR_CENTER) &&
				oldTileType != TILE_FLOOR_EDGE &&
				oldTileType != TILE_FLOOR_CENTER
			)
				room.freeTiles++;
			else if (
				tile != TILE_FLOOR_EDGE &&
				tile != TILE_FLOOR_CENTER &&
				(oldTileType == TILE_FLOOR_EDGE || oldTileType == TILE_FLOOR_CENTER)
			)
				room.freeTiles--;
			return true;
		}
	...
	this.Map.prototype.carve = function (room) {
		...
		for (var i in room.tiles) {
			...
			if (this.data[x][y][1] != -1 && (type == TILE_WALL || type == TILE_WALL_CORNER)) {
				...
				if (x > 1 && y > 1 && x < this.w - 2 && y < this.h - 2 && type == TILE_WALL)
					this.freeWalls.push([x, y]);
		...
		if (!door) {
			...
			if (!side) {
				...
				if (room.tiles.length > 0 && room.freeTiles > 0) {
					...
					if (this.isFloor(door[0] + side[0], door[1] + side[1]) != -1)
						this.removeFreeTile(door[0] + side[0], door[1] + side[1]);
					if (this.isFloor(door[0] - side[0], door[1] - side[1]) != -1)
						this.removeFreeTile(door[0] - side[0], door[1] - side[1]);
					room.parent = parentRoom;
					parentRoom.children.push(room);
					room.gen = parentRoom.gen + 1;
				} //not a good spot; remove this tile from the list of walls
				else {
					this.freeWalls.splice(this.isWall(door[0], door[1]), 1);
					success = 0;
				}
	...
	this.Map.prototype.getRandomSpotInRoom = function (room) {
		...
		for (var i in room.tiles) {
			if (
				(room.tiles[i].type == TILE_FLOOR_EDGE || room.tiles[i].type == TILE_FLOOR_CENTER) &&
				this.isFloor(room.tiles[i].x, room.tiles[i].y) != -1
			) {
	...
	this.Map.prototype.getDeepestRoom = function () {
		...
		for (var i in this.rooms) {
			if (
				this.rooms[i].gen + Math.sqrt(this.rooms[i].freeTiles) * 0.05 >= deepest &&
				this.rooms[i].corridor == 0 &&
				this.rooms[i].freeTiles > 4
			) {
	...
	this.Map.prototype.dig = function () {
		...
		if (this.digs == 0) {
			//first dig : build a starting room in the middle of the map
			var w = rand(3, 7);
			var h = rand(3, 7);
		...
		if (finished == 1) {
			//last touch : try to add a whole room at the end
			for (var i = 0; i < 10; i++) {
		...
		for (var i in this.rooms) {
			...
			for (var ii in this.rooms[i].tiles) {
				...
				if (complete) {
					...
					var bottom = 0;
					if (
						(xy1 == TILE_WALL || xy1 == TILE_WALL_CORNER) &&
						(y1 == TILE_WALL || y1 == TILE_WALL_CORNER) &&
						(xy2 == TILE_WALL || xy2 == TILE_WALL_CORNER)
					)
						top = 1;
					else if (
						(xy1 == TILE_FLOOR_CENTER || xy1 == TILE_FLOOR_EDGE) &&
						(y1 == TILE_FLOOR_CENTER || y1 == TILE_FLOOR_EDGE) &&
						(xy2 == TILE_FLOOR_CENTER || xy2 == TILE_FLOOR_EDGE)
					)
						top = -1;
					if (
						(xy2 == TILE_WALL || xy2 == TILE_WALL_CORNER) &&
						(x2 == TILE_WALL || x2 == TILE_WALL_CORNER) &&
						(xy4 == TILE_WALL || xy4 == TILE_WALL_CORNER)
					)
						right = 1;
					else if (
						(xy2 == TILE_FLOOR_CENTER || xy2 == TILE_FLOOR_EDGE) &&
						(x2 == TILE_FLOOR_CENTER || x2 == TILE_FLOOR_EDGE) &&
						(xy4 == TILE_FLOOR_CENTER || xy4 == TILE_FLOOR_EDGE)
					)
						right = -1;
					if (
						(xy1 == TILE_WALL || xy1 == TILE_WALL_CORNER) &&
						(x1 == TILE_WALL || x1 == TILE_WALL_CORNER) &&
						(xy3 == TILE_WALL || xy3 == TILE_WALL_CORNER)
					)
						left = 1;
					else if (
						(xy1 == TILE_FLOOR_CENTER || xy1 == TILE_FLOOR_EDGE) &&
						(x1 == TILE_FLOOR_CENTER || x1 == TILE_FLOOR_EDGE) &&
						(xy3 == TILE_FLOOR_CENTER || xy3 == TILE_FLOOR_EDGE)
					)
						left = -1;
					if (
						(xy3 == TILE_WALL || xy3 == TILE_WALL_CORNER) &&
						(y2 == TILE_WALL || y2 == TILE_WALL_CORNER) &&
						(xy4 == TILE_WALL || xy4 == TILE_WALL_CORNER)
					)
						bottom = 1;
					else if (
						(xy3 == TILE_FLOOR_CENTER || xy3 == TILE_FLOOR_EDGE) &&
						(y2 == TILE_FLOOR_CENTER || y2 == TILE_FLOOR_EDGE) &&
						(xy4 == TILE_FLOOR_CENTER || xy4 == TILE_FLOOR_EDGE)
					)
						bottom = -1;
					if (
				...
				if (
					(me != TILE_FLOOR_CENTER && me != TILE_FLOOR_EDGE) ||
					x1 == TILE_DOOR ||
					x2 == TILE_DOOR ||
					y1 == TILE_DOOR ||
					y2 == TILE_DOOR
	...
	this.Map.prototype.draw = function (size) {
		...
		for (var y = 0; y < this.h; y++) {
			for (var x = 0; x < this.w; x++) {
				...
				str +=
					...
					'px;top:' +
					y * size +
					'px;display:block;padding:0px;margin:0px;background:#' +
					colors[this.data[x][y][0]] +
					';color:#999;" title="' +
					title +
					'">' +
					text +
		...
		str =
			...
			'px;height:' +
			this.h * size +
			'px;background:#000;font-family:Courier;font-size:' +
			size +
			'px;float:left;margin:10px;">' +
			str +
			'</div>';
		return str;
	};
	...
	this.Map.prototype.drawDetailed = function () {
		...
		str =
			'<div style="box-shadow:0px 0px 12px 6px #00061b;position:relative;width:' +
			this.w * size +
			'px;height:' +
			this.h * size +
			'px;background:#00061b;font-family:Courier;font-size:' +
			size +
			'px;float:left;margin:10px;">' +
			str +
			'</div>';

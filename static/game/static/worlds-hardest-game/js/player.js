...
function playerMove() {
	if (state == 'game' && !player.dying && (!player.bouncing || player.bounceState >= 4)) {
		...
		if (keyRight) {
			...
			while (
				walls[level][player.tileTop][
					Math.floor((player.right + distance + WALL_BORDER_LEFT) / TILE_SIZE)
				] == 1 ||
				walls[level][player.tileBottom][
					Math.floor((player.right + distance + WALL_BORDER_LEFT) / TILE_SIZE)
				] == 1
			) {
				distance--;
			...
			if (distance > 0) {
				...
				while (
					distance < PLAYER_SPEED &&
					walls[level][Math.floor((player.bottom - distance + WALL_BORDER_TOP) / TILE_SIZE)][
						Math.floor((player.right + PLAYER_SPEED + WALL_BORDER_LEFT) / TILE_SIZE)
					] == 1 &&
					walls[level][
						Math.floor((player.bottom - CARVE - distance + WALL_BORDER_TOP) / TILE_SIZE)
					][Math.floor((player.right + PLAYER_SPEED + WALL_BORDER_LEFT) / TILE_SIZE)] == 0
				) {
					distance++;
				}
				if (distance > 0) {
				...
				while (
					distance < PLAYER_SPEED &&
					walls[level][Math.floor((player.top + distance - WALL_BORDER_BOTTOM) / TILE_SIZE)][
						Math.floor((player.right + PLAYER_SPEED + WALL_BORDER_LEFT) / TILE_SIZE)
					] == 1 &&
					walls[level][
						Math.floor((player.top + CARVE + distance - WALL_BORDER_BOTTOM) / TILE_SIZE)
					][Math.floor((player.right + PLAYER_SPEED + WALL_BORDER_LEFT) / TILE_SIZE)] == 0
				) {
					distance++;
				}
		...
		// left
			...
			while (
				walls[level][player.tileTop][
					Math.floor((player.left - distance - WALL_BORDER_RIGHT) / TILE_SIZE)
				] == 1 ||
				walls[level][player.tileBottom][
					Math.floor((player.left - distance - WALL_BORDER_RIGHT) / TILE_SIZE)
				] == 1
			...
			if (distance > 0) {
				...
				while (
					distance < PLAYER_SPEED &&
					walls[level][Math.floor((player.bottom - distance + WALL_BORDER_TOP) / TILE_SIZE)][
						Math.floor((player.left - PLAYER_SPEED - WALL_BORDER_RIGHT) / TILE_SIZE)
					] == 1 &&
					walls[level][
						Math.floor((player.bottom - CARVE - distance + WALL_BORDER_TOP) / TILE_SIZE)
					][Math.floor((player.left - PLAYER_SPEED - WALL_BORDER_RIGHT) / TILE_SIZE)] == 0
				) {
					distance++;
				}
				if (distance > 0) {
		...
		if (keyUp) {
			...
			if (distance == 0) {
				...
				if (!player.movedRight && !player.movedLeft) {
					...
					while (
						distance < PLAYER_SPEED &&
						walls[level][Math.floor((player.top - PLAYER_SPEED - WALL_BORDER_BOTTOM) / TILE_SIZE)][
							Math.floor((player.left + distance - WALL_BORDER_RIGHT) / TILE_SIZE)
						] == 1 &&
						walls[level][Math.floor((player.top - PLAYER_SPEED - WALL_BORDER_BOTTOM) / TILE_SIZE)][
							Math.floor((player.left + CARVE + distance - WALL_BORDER_RIGHT) / TILE_SIZE)
						] == 0
					) {
						distance++;
					}
				...
				if (!player.movedRight && !player.movedLeft) {
					...
					while (
						...
						walls[level][Math.floor((player.top - PLAYER_SPEED - WALL_BORDER_BOTTOM) / TILE_SIZE)][
							Math.floor((player.right - CARVE - distance + WALL_BORDER_LEFT) / TILE_SIZE)
						] == 0
					) {
						distance++;
					}
		...
		// down
			...
			if (distance == 0) {
				...
				if (!player.movedRight && !player.movedLeft) {
					...
					while (
						distance < PLAYER_SPEED &&
						walls[level][Math.floor((player.bottom + PLAYER_SPEED + WALL_BORDER_TOP) / TILE_SIZE)][
							Math.floor((player.left + distance - WALL_BORDER_RIGHT) / TILE_SIZE)
						] == 1 &&
						walls[level][Math.floor((player.bottom + PLAYER_SPEED + WALL_BORDER_TOP) / TILE_SIZE)][
							Math.floor((player.left + CARVE + distance - WALL_BORDER_RIGHT) / TILE_SIZE)
...
function playerHitEnemy() {
	...
	if (
		state == 'game' &&
		!invincible &&
		!invincible_permanent &&
		!playerFullOnCheck() &&
		!player.dying &&
		(!player.bouncing || player.bounceState >= 4)
	) {
...
function coinFullOnCheck(coinNum, checkNum) {
	if (state == 'game') {
		if (
			coins[level][coinNum].x + COIN_SIZE / 2 <
				(checkpoints[level][checkNum][0] + checkpoints[level][checkNum][2]) * TILE_SIZE &&
			coins[level][coinNum].x - COIN_SIZE / 2 > checkpoints[level][checkNum][0] * TILE_SIZE &&
			coins[level][coinNum].y + COIN_SIZE / 2 <
				(checkpoints[level][checkNum][1] + checkpoints[level][checkNum][3]) * TILE_SIZE &&
...
function drawPlayer() {
	if (state == 'game') {
		...
		if (player.bouncing) {
			...
			canvas.rect(
				cwh(
					player.x - (PLAYER_SIZE / 2) * ((player.bounceY / player.bounceTarget) * player.width)
				) + os.x,
				cwh(
					player.bounceTarget -
						yDif / 10 -
						PLAYER_SIZE * (((player.bounceY / player.bounceTarget) * player.height) / 1.5)
				) + os.y,
			...
			canvas.fillStyle =
				SHADOW_COLOR + (player.bounceY / player.bounceTarget) * SHADOW_OPACITY * player.alpha + ')';
			canvas.fill();
		}
		...
		canvas.beginPath();
		canvas.rect(
			cwh(player.x - (PLAYER_SIZE / 2) * player.width) + os.x,
		...
		canvas.rect(
			...
			cwh(drawY - PLAYER_SIZE * player.height) + os.y,
			cwh(PLAYER_SIZE * player.width),
			cwh(OUTLINE_SIZE * player.height)
		); // top
		canvas.rect(
		...
		canvas.rect(
			cwh(player.x - (PLAYER_SIZE / 2) * player.width) + os.x,
			cwh(drawY - OUTLINE_SIZE * player.height) + os.y,
			cwh(PLAYER_SIZE * player.width),
			cwh(OUTLINE_SIZE * player.height)
		); // bottom
		canvas.fillStyle =
...
function playerAtCheck(fall, bounce) {
	...
	if (state == 'game') {
		...
		player.y = Math.ceil(
			checkpoints[level][curCheck][1] * TILE_SIZE + checkHeight / 2 + PLAYER_SIZE / 2
		);
		invincible = false;

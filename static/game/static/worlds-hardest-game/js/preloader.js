...
function updatePreloader() {
	if (loadedAssets >= TOTAL_ASSETS && loadBarAlpha > 0) {
		...
		if (loadBarAlpha < 0) loadBarAlpha = 0;
	}
}
...
function drawPreloader() {
	...
	if (loadedImages < TOTAL_IMAGES) {
		canvas.fillText(
			'Loading Images... (' + loadedImages + ' / ' + TOTAL_IMAGES + ')',
			cwh(CANVAS_WIDTH / 2 - loadBarWidth / 2) + os.x,
			cwh(245) + os.y
		);
	} else if (loadedSounds < TOTAL_SOUNDS) {
		canvas.fillText(
			'Loading Sounds... (' + loadedSounds + ' / ' + TOTAL_SOUNDS + ')',
			cwh(CANVAS_WIDTH / 2 - loadBarWidth / 2) + os.x,
			cwh(245) + os.y
		);
	} else {
	...
	if (onButton('pl_playGame') || loadBarAlpha > 0) {
		...
		canvas.rect(
			cwh(CANVAS_WIDTH / 2 - loadBarWidth / 2 + 0.5) + os.x,
			cwh(255 + 0.5) + os.y,
			cwh(Math.floor((loadedAssets / TOTAL_ASSETS) * loadBarWidth)),
			cwh(30)
		);
		canvas.fill();
	}
	...
	canvas.textAlign = 'center';
	canvas.fillText(
		"This is The World's Hardest Game.",
		cwh(CANVAS_WIDTH / 2) + os.x,
		cwh(310) + os.y
	);
	canvas.fillText(
		'It is harder than any game you have',
		cwh(CANVAS_WIDTH / 2) + os.x,
		cwh(330) + os.y
	);

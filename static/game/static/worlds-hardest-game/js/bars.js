function drawBars() {
	if (mobile) {
		...
		if (state == 'game' || state == 'intermission') {
			...
			canvas.fillText(
				'LEVEL: ' + level,
				cwh(BAR_TEXT_FIX) + os.x,
				cwh(BAR_HEIGHT / 2 + BAR_TEXT_FIX) + os.y
			);
			...
			if (getCoinsTotal() > 0) {
				...
				canvas.fillText(
					'COINS: ' + getCoinsCollected() + '/' + getCoinsTotal(),
			...
			canvas.fillText(
				'FAILS: ' + numberWithCommas(deaths),
				cwh(CANVAS_WIDTH - BAR_TEXT_FIX) + os.x,
				cwh(BAR_HEIGHT / 2 + BAR_TEXT_FIX) + os.y
			);
			...
			if (muteSFX && muteMusic) {
				canvas.fillText(
					'UNMUTE',
					cwh(CANVAS_WIDTH - BAR_TEXT_FIX) + os.x,
					cwh(CANVAS_HEIGHT - BAR_HEIGHT / 2 + BAR_TEXT_FIX) + os.y
				);
		...
		if (state == 'game' || state == 'intermission') {
			...
			if (muteSFX && muteMusic) {
				canvas.fillText(
					'UNMUTE',
					CANVAS_WIDTH - BAR_TEXT_FIX,
					CANVAS_HEIGHT - BAR_HEIGHT / 2 + BAR_TEXT_FIX
				);

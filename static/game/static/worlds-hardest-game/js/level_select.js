...
function LS_draw_player(l, x, y) {
	...
	canvas.rect(
		cwh(
			x +
				Math.floor(
					(checkpoints[l][0][0] + checkpoints[l][0][2] / 2) * LS_TILE_SIZE - LS_PLAYER_SIZE / 2
				)
		) + os.x,
		cwh(
			y +
				Math.floor(
					(checkpoints[l][0][1] + checkpoints[l][0][3] / 2) * LS_TILE_SIZE - LS_PLAYER_SIZE / 2
				)
		) + os.y,
		cwh(LS_PLAYER_SIZE),
		cwh(LS_PLAYER_SIZE)
...
function LS_draw_enemies(l, x, y) {
	...
	for (var i = 0; i < enemies[l].length; i++) {
		...
		canvas.rect(
			cwh(
				x + Math.floor(enemies[l][i].simpleX * LS_TILE_SIZE - (LS_TILE_SIZE - LS_COIN_SIZE) / 2)
			) + os.x,
			cwh(
				y + Math.floor(enemies[l][i].simpleY * LS_TILE_SIZE - (LS_TILE_SIZE - LS_COIN_SIZE) / 2)
			) + os.y,
			cwh(LS_ENEMY_SIZE),
			cwh(LS_ENEMY_SIZE)
		);
...
function LS_draw_coins(l, x, y) {
	for (var i = 0; i < coins[l].length; i++) {
		canvas.beginPath();
		canvas.rect(
			cwh(x + Math.floor(coins[l][i].simpleX * LS_TILE_SIZE + (LS_TILE_SIZE - LS_COIN_SIZE) / 2)) +
				os.x,
			cwh(y + Math.floor(coins[l][i].simpleY * LS_TILE_SIZE + (LS_TILE_SIZE - LS_COIN_SIZE) / 2)) +
				os.y,
			cwh(LS_COIN_SIZE),
			cwh(LS_COIN_SIZE)
		);
...
function LS_draw_text(l, x, y) {
	if (onButton('ls_' + l)) canvas.fillStyle = LS_BORDER_HOVER_COLOR;
...
function LS_draw_walls(l, x, y) {
	...
	canvas.rect(
		cwh(x) + os.x,
		cwh(y) + os.y,
		cwh(LS_TILE_SIZE * TILES_X),
		cwh(LS_TILE_SIZE * TILES_Y)
	);
	...
	for (var i = 0; i < LS_TILES_HEIGHT; i++) {
		for (var j = 0; j < LS_TILES_WIDTH; j++) {
			if (walls[l][i][j] == 0) {
				...
				if ((j % 2 == 0 && i % 2 == 0) || (j % 2 == 1 && i % 2 == 1)) bgTileColor = floorColor1;
				canvas.beginPath();
				canvas.rect(
					cwh(x + j * LS_TILE_SIZE) + os.x,
					cwh(y + i * LS_TILE_SIZE) + os.y,
...
function LS_draw_border(l, x, y) {
	...
	canvas.beginPath();
	canvas.rect(
		cwh(x - LS_BDR_SIZE + LS_SHADOW_OFFSET) + os.x,
		cwh(y - LS_BDR_SIZE + LS_SHADOW_OFFSET) + os.y,
		cwh(LS_TILE_SIZE * LS_TILES_WIDTH + LS_BDR_SIZE * 2),
		cwh(LS_TILE_SIZE * LS_TILES_HEIGHT + LS_BDR_SIZE * 2)
	);
	...
	canvas.beginPath();
	canvas.rect(
		cwh(x - LS_BDR_SIZE) + os.x,
		cwh(y - LS_BDR_SIZE) + os.y,
		cwh(LS_TILE_SIZE * LS_TILES_WIDTH + LS_BDR_SIZE * 2),
		cwh(LS_TILE_SIZE * LS_TILES_HEIGHT + LS_BDR_SIZE * 2)
	);
	if (onButton('ls_' + l)) canvas.fillStyle = LS_BORDER_HOVER_COLOR;
	else canvas.fillStyle = LS_BORDER_COLOR;
	canvas.fill();
}
...
function LS_draw_nav() {
	// back
	if (ls_page == 2) {
		if (onButton('ls_back')) canvas.fillStyle = LS_BUTTON_HOVER_COLOR;
		else canvas.fillStyle = 'black';
	} else {
	...
	// back to menu
	if (onButton('ls_menu')) canvas.fillStyle = LS_BUTTON_HOVER_COLOR;
	else canvas.fillStyle = 'black';
	canvas.font = cwh(25) + 'px Arial Black';
	canvas.textAlign = 'center';
	...
	// next
	if (ls_page == 1) {
		if (onButton('ls_next')) canvas.fillStyle = LS_BUTTON_HOVER_COLOR;
		else canvas.fillStyle = 'black';
	} else {
		canvas.fillStyle = LS_BUTTON_DISABLED_COLOR;
	}

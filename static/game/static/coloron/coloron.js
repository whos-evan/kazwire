class Game {
	...
	intro() {
		...
		ball
			.fromTo(
				...
				0.5,
				{ y: '0px' },
				{ y: '100px', scaleY: 1.1, transformOrigin: 'bottom', ease: Power2.easeIn }
			)
	 ...
	 */
	showGrade(score) {
		if (score > 30) return 'Godlike!';
		else if (score > 25) return 'GG!';
		else if (score > 20) return 'Epic!';
...
class Animation {
	...
	playTriangle(el) {
		...
		triangle.staggerFromTo(el.find('.triangle'), 0.3, { scale: 0.1 }, { scale: 1 }, 0.03).staggerTo(
			el.find('.triangle'),
			1.5,
			{
				cycle: {
	...
	playBlock(el) {
		...
		block.staggerFromTo(el.find('.block'), 0.3, { scale: 0.1 }, { scale: 1 }, 0.03).staggerTo(

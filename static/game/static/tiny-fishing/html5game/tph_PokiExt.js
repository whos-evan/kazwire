...
var poki_callbackPause = function () {
	console.log('Callback PAUSE');
};
var poki_callbackUnpause = function () {
	console.log('Callback UNPAUSE');
};
...
function poki_init(_callbackPause, _callbackUnpause, prod) {
	...
	if (prod) {
		if (window.PokiSDK) PokiSDK.setDebug(false);
		//sl();
	} else {
		if (window.PokiSDK) PokiSDK.setDebug(true);
	}
}

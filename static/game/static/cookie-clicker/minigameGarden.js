...
M.launch = function () {
		...
		*/
		M.plants = {
			...
			gildmillet: {
				...
				effsStr:
					'<div class="green">&bull; +1% golden cookie gains</div><div class="green">&bull; +0.1% golden cookie effect duration</div>',
			...
			bakeberry: {
				...
				children: ['queenbeet'],
				effsStr:
					'<div class="green">&bull; +1% CpS</div><div class="green">&bull; harvest when mature for +30 minutes of CpS (max. 3% of bank)</div>',
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						...
						if (moni != 0) {
							Game.Earn(moni);
							Game.Popup(
								'(Bakeberry)<br>+' + Beautify(moni) + ' cookies!',
								Game.mouseX,
								Game.mouseY
							);
						}
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						...
						if (moni != 0) {
							Game.Earn(moni);
							Game.Popup(
								'(Chocoroot)<br>+' + Beautify(moni) + ' cookies!',
								Game.mouseX,
								Game.mouseY
							);
						}
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						...
						if (moni != 0) {
							Game.Earn(moni);
							Game.Popup(
								'(White chocoroot)<br>+' + Beautify(moni) + ' cookies!',
								Game.mouseX,
								Game.mouseY
							);
						}
			...
			whiteMildew: {
				...
				effsStr:
					'<div class="green">&bull; +1% CpS</div><div class="gray">&bull; may spread as brown mold</div>',
			...
			brownMold: {
				...
				effsStr:
					'<div class="red">&bull; -1% CpS</div><div class="gray">&bull; may spread as white mildew</div>',
			...
			chimerose: {
				...
				children: ['chimerose'],
				effsStr:
					'<div class="green">&bull; +1% reindeer gains</div><div class="green">&bull; +1% reindeer frequency</div>',
			...
			nursetulip: {
				...
				children: [],
				effsStr:
					'<div class="green">&bull; surrounding plants (3x3) are 20% more efficient</div><div class="red">&bull; -2% CpS</div>',
			...
			wardlichen: {
				...
				children: ['wardlichen'],
				effsStr:
					'<div class="gray">&bull; 2% less wrath cookies</div><div class="gray">&bull; wrinklers spawn 15% slower</div>',
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						...
						if (moni != 0) {
							Game.Earn(moni);
							Game.Popup(
								'(Queenbeet)<br>+' + Beautify(moni) + ' cookies!',
								Game.mouseX,
								Game.mouseY
							);
						}
			...
			queenbeetLump: {
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						Game.gainLumps(1);
						popup =
							'(Juicy queenbeet)<br>Sweet!<div style="font-size:65%;">Found 1 sugar lump!</div>';
					}
				}
			},
			duketater: {
				...
				children: ['shriekbulb'],
				effsStr:
					'<div class="green">&bull; harvest when mature for +2 hours of CpS (max. 8% of bank)</div>',
				...
				onHarvest: function (x, y, age) {
					if (age >= this.mature) {
						...
						if (moni != 0) {
							Game.Earn(moni);
							Game.Popup(
								'(Duketater)<br>+' + Beautify(moni) + ' cookies!',
								Game.mouseX,
								Game.mouseY
							);
						}
			...
			crumbspore: {
				...
				onDie: function (x, y) {
					...
					if (moni != 0) {
						Game.Earn(moni);
						Game.Popup(
							'(Crumbspore)<br>+' + Beautify(moni) + ' cookies!',
							Game.mouseX,
							Game.mouseY
						);
					}
			...
			doughshroom: {
				...
				onDie: function (x, y) {
					...
					if (moni != 0) {
						Game.Earn(moni);
						Game.Popup(
							'(Doughshroom)<br>+' + Beautify(moni) + ' cookies!',
							Game.mouseX,
							Game.mouseY
						);
					}
			...
			wrinklegill: {
				...
				effsStr:
					'<div class="gray">&bull; wrinklers spawn 2% faster</div><div class="gray">&bull; wrinklers eat 1% more</div>',
			...
			shriekbulb: {
				...
				children: ['shriekbulb'],
				effsStr:
					'<div class="red">&bull; -2% CpS</div><div class="red">&bull; surrounding plants (3x3) are 5% less efficient</div>',
			...
			tidygrass: {
				...
				children: ['everdaisy'],
				effsStr:
					'<div class="green">&bull; surrounding tiles (5x5) develop no weeds or fungus</div>',
			...
			everdaisy: {
				...
				children: [],
				effsStr:
					'<div class="green">&bull; surrounding tiles (3x3) develop no weeds or fungus</div><div class="green">&bull; immortal</div>',
		...
		for (var i in M.plants) {
			...
			if (typeof M.plants[i].plantable === 'undefined') {
				M.plants[i].plantable = true;
			}
			n++;
		}
		...
		M.getUnlockedN = function () {
			M.plantsUnlockedN = 0;
			for (var i in M.plants) {
				if (M.plants[i].unlocked) M.plantsUnlockedN++;
			}
		...
		M.dropUpgrade = function (upgrade, rate) {
			if (
				!Game.Has(upgrade) &&
				Math.random() <= rate * Game.dropRateMult() * (Game.HasAchiev('Seedless to nay') ? 1.05 : 1)
			) {
				Game.Unlock(upgrade);
			}
		};
		...
		M.getMuts = function (neighs, neighsM) {
			...
			if (neighsM['bakerWheat'] >= 2)
				muts.push(['bakerWheat', 0.2], ['thumbcorn', 0.05], ['bakeberry', 0.001]);
			...
			if (neighsM['bakerWheat'] >= 1 && neighsM['gildmillet'] >= 1)
				muts.push(['clover', 0.03], ['goldenClover', 0.0007]);
			if (neighsM['clover'] >= 1 && neighsM['gildmillet'] >= 1) muts.push(['shimmerlily', 0.02]);
			if (neighsM['clover'] >= 2 && neighs['clover'] < 5)
				muts.push(['clover', 0.007], ['goldenClover', 0.0001]);
			...
			if (neighsM['chocoroot'] >= 1 && neighs['whiteMildew'] >= 1)
				muts.push(['whiteChocoroot', 0.1]);
			...
			if (neighsM['shimmerlily'] >= 1 && neighsM['whiteChocoroot'] >= 1)
				muts.push(['whiskerbloom', 0.01]);
			if (neighsM['shimmerlily'] >= 1 && neighsM['whiskerbloom'] >= 1)
				muts.push(['chimerose', 0.05]);
			...
			if (
				(neighsM['cronerice'] >= 1 && neighsM['keenmoss'] >= 1) ||
				(neighsM['cronerice'] >= 1 && neighsM['whiteMildew'] >= 1)
			)
			...
			if (neighsM['doughshroom'] >= 1 && neighs['doughshroom'] <= 1)
				muts.push(['doughshroom', 0.07]);
			...
			if (neighsM['wrinklegill'] >= 1 && neighsM['elderwort'] >= 1)
				muts.push(['shriekbulb', 0.001]);
			...
			if (neighsM['bakerWheat'] >= 1 && neighsM['whiteChocoroot'] >= 1)
				muts.push(['tidygrass', 0.002]);
		...
		M.soils = {
			...
			clay: {
				...
				req: 100,
				effsStr:
					'<div class="gray">&bull; tick every <b>15 minutes</b></div><div class="green">&bull; passive plant effects <b>+25%</b></div>',
		...
		M.tools = {
			info: {
				...
				descFunc: function () {
					...
					if (M.freeze) str = 'Your garden is frozen, providing no effects.';
						...
						for (var i in M.effs) {
							if (M.effs[i] != 1 && effs[i]) {
								...
								effStr +=
									...
									effs[i].n +
									' :</b> <span class="' +
									(amount * (effs[i].rev ? -1 : 1) > 0 ? 'green' : 'red') +
									'">' +
									(amount > 0 ? '+' : '-') +
						...
						if (effStr == '')
							effStr = '<div style="font-size:10px;margin-left:64px;"><b>None.</b></div>';
			...
			convert: {
				...
				isDisplayed: function () {
					if (M.plantsUnlockedN >= M.plantsN) return true;
					else return false;
				}
			}
		};
		...
		M.getCost = function (me) {
			...
			return (
				Math.max(me.costM, Game.cookiesPs * me.cost * 60) *
			...
			if (me.children.length > 0) {
				...
				for (var i in me.children) {
					if (!M.plants[me.children[i]]) console.log('No plant named ' + me.children[i]);
						...
						if (it.unlocked)
							children +=
								'<div class="gardenSeedTiny" style="background-position:' +
								-0 * 48 +
								'px ' +
								-it.icon * 48 +
								'px;"></div>';
			...
			return (
				...
				(!me.immortal
					? '<div style="margin:6px 0px;font-size:11px;"><b>Average lifespan :</b> ' +
					  Game.sayTime((100 / (me.ageTick + me.ageTickR / 2)) * M.stepT * 30, -1) +
					  ' <small>(' +
				...
				Game.sayTime(
					(100 / (me.ageTick + me.ageTickR / 2)) * (me.mature / 100) * M.stepT * 30,
					-1
				) +
				' <small>(' +
				...
				(me.detailsStr
					? '<div style="margin:6px 0px;font-size:11px;"><b>Details :</b> ' +
					  me.detailsStr +
					  '</div>'
					: '') +
				(children != ''
					? '<div style="margin:6px 0px;font-size:11px;"><b>Possible mutations :</b> ' +
					  children +
					  '</div>'
					: '') +
				'<div class="line"></div>' +
		...
		M.canPlant = function (me) {
			if (Game.cookies >= M.getCost(me)) return true;
			else return false;
		};
					...
					(M.parent.amount < me.req
						...
						: '<div class="icon" style="background:url(img/gardenPlants.png?v=' +
						  ...
						  (M.soil == me.id
								...
								: M.nextSoil > Date.now()
								? 'You will be able to change your soil again in ' +
								  Game.sayTime(((M.nextSoil - Date.now()) / 1000) * 30 + 30, -1) +
								  '.'
			...
			return function () {
				...
				var str =
					...
					(me.plantable
						...
						: '<span class="red">This seed cannot be planted.</span>') +
					...
					var str =
						...
						(me
							? '<div class="line"></div>Click to plant <b>' +
							  ...
							  (M.canPlant(me) ? '' : ' disabled') +
							  '">' +
							  Beautify(Math.round(M.getCost(me))) +
							  '</span>.<br><small>(Shift-click to plant multiple.)</small><br><small>(Holding the shift key pressed will also hide tooltips.)</small>'
							: '') +
						(M.plotBoost[y][x] != [1, 1, 1]
							? '<small>' +
							  (M.plotBoost[y][x][0] != 1
									? '<br>Aging multiplier : ' + Beautify(M.plotBoost[y][x][0] * 100) + '%'
									: '') +
							  (M.plotBoost[y][x][1] != 1
									? '<br>Effect multiplier : ' + Beautify(M.plotBoost[y][x][1] * 100) + '%'
									: '') +
							  (M.plotBoost[y][x][2] != 1
									? '<br>Weeds/fungus repellent : ' +
									  Beautify(100 - M.plotBoost[y][x][2] * 100) +
									  '%'
									: '') +
							  '</small>'
							: '') +
		...
		M.buildPanel = function () {
			...
			for (var i in M.plants) {
				...
				str +=
					...
					me.id +
					'" class="gardenSeed' +
					(M.seedSelected == me.id ? ' on' : '') +
					' locked" ' +
					Game.getDynamicTooltip(
						'Game.ObjectsById[' + M.parent.id + '].minigame.seedTooltip(' + me.id + ')',
						'this'
					) +
					'>';
				str +=
			...
			for (var i in M.tools) {
				...
				str +=
					...
					'" ' +
					Game.getDynamicTooltip(
						'Game.ObjectsById[' + M.parent.id + '].minigame.toolTooltip(' + me.id + ')',
						'this'
					) +
					'>';
				str +=
			...
			for (var i in M.soils) {
				...
				str +=
					...
					'" ' +
					Game.getDynamicTooltip(
						'Game.ObjectsById[' + M.parent.id + '].minigame.soilTooltip(' + me.id + ')',
						'this'
					) +
					'>';
				str +=
				...
				for (var y = 0; y < 6; y++) {
					for (var x = 0; x < 6; x++) {
						str +=
							...
							'>';
						str +=
							'<div id="gardenTileIcon-' +
							x +
							'-' +
							y +
							'" class="gardenTileIcon" style="display:none;"></div>';
						str += '</div>';
					}
				}
		...
		M.isTileUnlocked = function (x, y) {
			...
			if (x >= limits[0] && x < limits[2] && y >= limits[1] && y < limits[3]) return true;
			else return false;
		};
		...
		M.askConvert = function () {
			...
			Game.Prompt(
				...
				[
					['Yes!', 'Game.ClosePrompt();Game.ObjectsById[' + M.parent.id + '].minigame.convert();'],
					'No'
				]
			);
		};
		...
		M.harvest = function (x, y, manual) {
			...
			if (tile[0] >= 1) {
				...
				if (tile[1] >= me.mature) {
					if (M.unlockSeed(me))
						Game.Popup(
							'(' + me.name + ')<br>Unlocked ' + me.name + ' seed.',
							Game.mouseX,
							Game.mouseY
						);
					M.harvests++;
		...
		str +=
			'<div id="gardenSeedsUnlocked" class="title gardenPanelLabel">Seeds</div><div class="line"></div>';
		str += '<div id="gardenSeeds"></div>';
		str += '</div>';
		str += '<div id="gardenField">';
		str +=
			'<div style="pointer-events:none;opacity:0.75;position:absolute;left:0px;right:0px;top:8px;" id="gardenPlotSize"></div>';
		str +=
			'<div id="gardenPlot" class="shadowFilter" style="width:' +
			6 * M.tileSize +
			'px;height:' +
			6 * M.tileSize +
			'px;"></div>';
	...
	M.onResize = function () {
		...
		var panelW = Math.min(Math.max(width * 0.4, 320), width - 6 * M.tileSize) - 8;
		var fieldW = Math.max(Math.min(width * 0.6, width - panelW), 6 * M.tileSize) - 8;
					...
					for (var x = 0; x < 6; x++) {
						if (M.isTileUnlocked(x, y)) {
							...
							if (tile[0] > 0) {
								//age
								tile[1] += randomFloor(
									(me.ageTick + me.ageTickR * Math.random()) * M.plotBoost[y][x][0]
								);
									...
									if (any > 0) {
										...
										for (var ii = 0; ii < muts.length; ii++) {
											if (
												...
												(!M.plants[muts[ii][0]].weed || Math.random() < weedMult) &&
												((!M.plants[muts[ii][0]].weed && !M.plants[muts[ii][0]].fungus) ||
													Math.random() < M.plotBoost[y][x][2])
											)
												list.push(muts[ii][0]);
										}
	...
	M.draw = function () {
		...
		if (Game.drawT % 10 == 0) {
			...
			if (M.freeze) l('gardenNextTick').innerHTML = 'Garden is frozen. Unfreeze to resume.';
				...
				l('gardenNextTick').innerHTML =
					'Next tick in ' + Game.sayTime(((M.nextStep - Date.now()) / 1000) * 30 + 30, -1) + '';
			l('gardenStats').innerHTML =
				'Mature plants harvested : ' +
				Beautify(M.harvests) +
				' (total : ' +
				Beautify(M.harvestsTotal) +
				')';
			...
			l('gardenSeedsUnlocked').innerHTML =
				'Seeds<small> (' + M.plantsUnlockedN + '/' + M.plantsN + ')</small>';
			for (var i in M.soils) {
				var me = M.soils[i];

...
M.launch = function () {
	...
	M.init = function (div) {
		...
		M.computeMagicM = function () {
			...
			M.magicM = Math.floor(
				4 + Math.pow(towers, 0.6) + Math.log((towers + (lvl - 1) * 10) / 15 + 1) * 15
			);
			//old formula :
			/*
		...
		for (var i in M.spells) {
			...
			str +=
				...
				Game.getDynamicTooltip(
					'Game.ObjectsById[' + M.parent.id + '].minigame.spellTooltip(' + me.id + ')',
					'this'
				) +
	...
	M.draw = function () {
		...
		M.infoL.innerHTML =
			'Spells cast : ' + Beautify(M.spellsCast) + ' (total : ' + Beautify(M.spellsCastTotal) + ')';
	};

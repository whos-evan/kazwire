...
const broadcastChannel =
	typeof BroadcastChannel === 'undefined' ? null : new BroadcastChannel(BROADCASTCHANNEL_NAME);
function PostBroadcastMessage(o) {
	if (!broadcastChannel) return;
	setTimeout(() => broadcastChannel.postMessage(o), 3e3);
}
...
function IsUrlInLazyLoadList(url, lazyLoadList) {
	...
	try {
		for (const lazyLoadRegex of lazyLoadList) if (new RegExp(lazyLoadRegex).test(url)) return true;
	} catch (err) {
...
function WriteLazyLoadListToStorage(lazyLoadList) {
	...
	else return localforage.setItem(LAZYLOAD_KEYNAME, lazyLoadList);
}
function ReadLazyLoadListFromStorage() {
	if (typeof localforage === 'undefined') return Promise.resolve([]);
	else return localforage.getItem(LAZYLOAD_KEYNAME);
}
...
async function UpdateCheck(isFirst) {
	try {
		...
		if (!response.ok)
			throw new Error(
				OFFLINE_DATA_FILE + ' responded with ' + response.status + ' ' + response.statusText
			);
...
async function GetCacheNameToUse(availableCacheNames, doUpdateCheck) {
	if (availableCacheNames.length === 1 || !doUpdateCheck) return availableCacheNames[0];
...
self.addEventListener('fetch', (event) => {
	...
	if (doUpdateCheck) event.waitUntil(responsePromise.then(() => UpdateCheck(false)));

...
window.DOMHandler = class {
	...
	_StartTicking() {
		this._hasTickCallback ||
			(this._iRuntime._AddRAFCallback(this._tickCallback), (this._hasTickCallback = !0));
	}
	_StopTicking() {
		this._hasTickCallback &&
	...
	Call() {
		if (-1 === this._timerId) {
			var d = Date.now(),
				...
				b = this._interval;
			a >= b && this._canRunImmediate
				? ((this._lastCallTime = d), this._RunCallback())
				: (this._timerId = self.setTimeout(this._timerCallFunc, Math.max(b - a, 4)));
		}
	}
	_RunCallback() {
		this._ignoreReset = !0;
...
window.DOMElementHandler = class extends self.DOMHandler {
	constructor(d, a) {
		...
		this.AddRuntimeMessageHandlers([
			...
			['remove-attribute', (b) => this._OnRemoveAttribute(b)]
		]);
...
('use strict');
{
	...
	function b(g, c) {
		...
		k.async = !1;
		'module' === c && (k.type = 'module');
		return g.isStringSrc
			? new Promise((l) => {
	...
	window.RuntimeInterface = class g {
		constructor(c) {
			...
			this._scriptsType = c.scriptsType;
			...
			this.AddRuntimeComponentMessageHandler('runtime', 'cordova-fetch-local-file', (k) =>
				this._OnCordovaFetchLocalFile(k)
			);
			this.AddRuntimeComponentMessageHandler('runtime', 'create-job-worker', (k) =>
				this._OnCreateJobWorker(k)
			);
			'cordova' === this._exportType
				? document.addEventListener('deviceready', () => this._Init(c))
				: this._Init(c);
		}
		Release() {
		...
		IsiOSCordova() {
			return d && 'cordova' === this._exportType;
		}
		IsiOSWebView() {
		...
		async _Init(c) {
			if ('playable-ad' === this._exportType) {
				...
				for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
					var k = c.engineScripts[l].toLowerCase();
					this._localFileStrings.hasOwnProperty(k)
						? (c.engineScripts[l] = { isStringSrc: !0, str: this._localFileStrings[k] })
						: this._localFileBlobs.hasOwnProperty(k) &&
						  (c.engineScripts[l] = URL.createObjectURL(this._localFileBlobs[k]));
				}
			}
			c.baseUrl
			...
			window.c3_addPortMessageHandler &&
				window.c3_addPortMessageHandler((l) => this._OnMessageFromDebugger(l));
			...
			this._useWorker ? await this._InitWorker(c, k.port2) : await this._InitDOM(c, k.port2);
		}
		_GetWorkerURL(c) {
			c = this._workerScriptURLs.hasOwnProperty(c)
				? this._workerScriptURLs[c]
				: c.endsWith('/workermain.js') && this._workerScriptURLs.hasOwnProperty('workermain.js')
				? this._workerScriptURLs['workermain.js']
				: 'playable-ad' === this._exportType && this._localFileBlobs.hasOwnProperty(c.toLowerCase())
				? this._localFileBlobs[c.toLowerCase()]
				: c;
			c instanceof Blob && (c = URL.createObjectURL(c));
			return c;
		}
		async CreateWorker(c, k, l) {
			if (c.startsWith('blob:')) return new Worker(c, l);
			if (this.IsiOSCordova() && 'file:' === location.protocol)
				return (
					(c = await this.CordovaFetchLocalFileAsArrayBuffer(this._scriptFolder + c)),
		...
		async _InitWorker(c, k) {
			...
			this._worker.postMessage(
				Object.assign(this._GetCommonRuntimeOptions(c), {
					type: 'init-runtime',
					isInWorker: !0,
					messagePort: k,
					canvas: l,
			...
			this._domHandlers = B.map((n) => new n(this));
			this._FindRuntimeDOMHandler();
			self.c3_callFunction = (n, q) => this._runtimeDomHandler._InvokeFunctionFromJS(n, q);
		...
		async _InitDOM(c, k) {
			...
			document.body.appendChild(this._canvas);
			window.c3canvas = this._canvas;
			this._domHandlers = B.map((q) => new q(this));
			this._FindRuntimeDOMHandler();
			var l = c.engineScripts.map((q) =>
				'string' === typeof q ? new URL(q, this._baseUrl).toString() : q
			);
			Array.isArray(c.workerDependencyScripts) && l.unshift(...c.workerDependencyScripts);
			l = await Promise.all(l.map((q) => this._MaybeGetCordovaScriptURL(q, this._scriptsType)));
			await Promise.all(l.map((q) => b(q, this._scriptsType)));
			const n = self.C3_ProjectScriptsStatus;
			if ('module' === this._scriptsType) {
				l = c.mainProjectScript;
				const q = c.projectScripts;
				for (let [t, r] of q)
					if ((r || (r = t), t === l))
						try {
							(r = await this._MaybeGetCordovaScriptURL(r, this._scriptsType)),
								await b(r, this._scriptsType),
								'preview' !== this._exportType ||
									n[t] ||
									this._ReportProjectMainScriptError(t, 'main script did not run to completion');
						} catch (y) {
							this._ReportProjectMainScriptError(t, y);
						}
					else if ('scriptsInEvents.js' === t || t.endsWith('/scriptsInEvents.js'))
						(r = await this._MaybeGetCordovaScriptURL(r, this._scriptsType)),
							await b(r, this._scriptsType);
			} else if (c.projectScripts && 0 < c.projectScripts.length)
				try {
					if (
						(await Promise.all(c.projectScripts.map((q) => b(q[1], this._scriptsType))),
						Object.values(n).some((q) => !q))
					) {
						self.setTimeout(() => this._ReportProjectScriptError(n), 100);
						return;
					}
				} catch (q) {
					console.error('[Preview] Error loading project scripts: ', q);
					self.setTimeout(() => this._ReportProjectScriptError(n), 100);
					return;
				}
			'preview' === this._exportType && 'object' !== typeof self.C3.ScriptsInEvents
				...
				: ((c = Object.assign(this._GetCommonRuntimeOptions(c), {
				  ...
				  (this._localRuntime = self.C3_CreateRuntime(c)),
				  await self.C3_InitRuntime(this._localRuntime, c));
		}
		_ReportProjectScriptError(c) {
			this._RemoveLoadingMessage();
			c = `Failed to load project script '${
				Object.entries(c)
					.filter((k) => !k[1])
					.map((k) => k[0])[0]
			}'. Check all your JavaScript code has valid syntax.`;
			console.error('[Preview] ' + c);
			alert(c);
		}
		_ReportProjectMainScriptError(c, k) {
			this._RemoveLoadingMessage();
		...
		_RemoveLoadingMessage() {
			const c = window.cr_previewLoadingElem;
		...
		PostToRuntimeComponent(c, k, l, n, q) {
		...
		PostToRuntimeComponentAsync(c, k, l, n, q) {
			const t = D++,
				r = new Promise((y, G) => {
					C.set(t, { resolve: y, reject: G });
				});
			this._messageChannelPort.postMessage(
				{
					type: 'event',
		...
		['_OnMessageFromRuntime'](c) {
			const k = c.type;
			if ('event' === k) return this._OnEventFromRuntime(c);
			if ('result' === k) this._OnResultFromRuntime(c);
			else if ('runtime-ready' === k) this._OnRuntimeReady();
			else if ('alert-error' === k) this._RemoveLoadingMessage(), alert(c.message);
		...
		AddRuntimeComponentMessageHandler(c, k, l) {
			let n = x.get(c);
			n || ((n = new Map()), x.set(c, n));
			if (n.has(k)) throw Error(`[DOM] Component '${c}' already has handler '${k}'`);
			n.set(k, l);
		}
		static AddDOMHandlerClass(c) {
			if (B.includes(c)) throw Error('DOM handler already added');
			B.push(c);
		}
		...
		async _WasmDecodeWebMOpus(c) {
			c = await this.PostToRuntimeComponentAsync(
				'runtime',
				'opus-decode',
				{ arrayBuffer: c },
				null,
		...
		IsAbsoluteURL(c) {
			return (
				/^(?:[a-z\-]+:)?\/\//.test(c) || 'data:' === c.substr(0, 5) || 'blob:' === c.substr(0, 5)
			);
		}
		IsRelativeURL(c) {
			return !this.IsAbsoluteURL(c);
		}
		async _MaybeGetCordovaScriptURL(c, k) {
			return 'cordova' === this._exportType &&
				'module' === k &&
				(c.startsWith('file:') || ('file:' === location.protocol && this.IsRelativeURL(c)))
				? (c.startsWith(this._baseUrl) && (c = c.substr(this._baseUrl.length)),
				  (c = await this.CordovaFetchLocalFileAsArrayBuffer(c)),
				  (c = new Blob([c], { type: 'application/javascript' })),
		...
		_HasPermission(c) {
			...
			return new Promise((l, n) =>
				k.checkPermission(this._MapPermissionID(k, c), (q) => l(!!q.hasPermission), n)
			);
		}
		_RequestPermission(c) {
			const k = this._GetPermissionAPI();
			return new Promise((l, n) =>
				k.requestPermission(this._MapPermissionID(k, c), (q) => l(!!q.hasPermission), n)
			);
		}
		...
		async _CordovaDoFetchLocalFileAsAsArrayBuffer(c, k, l) {
			try {
		...
		async _ConvertDataUrisToBlobs() {
			const c = [];
			for (const [k, l] of Object.entries(this._localFileBlobs))
				c.push(this._ConvertDataUriToBlobs(k, l));
			await Promise.all(c);
		}
		async _ConvertDataUriToBlobs(c, k) {
			if ('object' === typeof k)
				(this._localFileBlobs[c] = new Blob([k.str], { type: k.type })),
					(this._localFileStrings[c] = k.str);
			else {
		...
		_ParseDataURI(c) {
			...
			var l = c.substring(5, k);
			c = c.substring(k + 1);
			k = l.split(';');
			l = k[0] || '';
			const n = k[2];
			c = 'base64' === k[1] || 'base64' === n ? atob(c) : decodeURIComponent(c);
			return { mime_type: l, data: c };
		}
		_BinaryStringToBlob(c, k) {
			var l = c.length;
			let n = l >> 2,
				q = new Uint8Array(l),
			...
			for (y = r = 0; r < n; ++r)
				t[r] =
					...
					(c.charCodeAt(y++) << 16) |
					(c.charCodeAt(y++) << 24);
			for (l &= 3; l--; ) (q[y] = c.charCodeAt(y)), ++y;
			return new Blob([q], { type: k });
		}
	};
}
('use strict');
{
	...
	function a(f) {
		return (
			...
			(f.originalEvent &&
				f.originalEvent.sourceCapabilities &&
				f.originalEvent.sourceCapabilities.firesTouchEvents)
		);
	}
	const b = new Map([
			['OSLeft', 'MetaLeft'],
		...
		e = { dispatchRuntimeEvent: !0, dispatchUserScriptEvent: !0 },
		h = { dispatchUserScriptEvent: !0 },
		m = { dispatchRuntimeEvent: !0 };
	function p(f) {
	...
	const B = new Set(['canvas', 'body', 'html']);
	function x(f) {
		const g = f.target.tagName.toLowerCase();
		B.has(g) && f.preventDefault();
	}
	function C(f) {
	...
	self.C3_RasterSvgImageBlob = async function (f, g, c, k, l) {
		f = await u(f, g, c);
		...
		class extends self.DOMHandler {
			constructor(f) {
				...
				this._lastPointerRawUpdateEvent =
					...
					this._debugHighlightElem =
						null;
				f.AddRuntimeComponentMessageHandler('canvas', 'update-size', (k) =>
					this._OnUpdateCanvasSize(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'invoke-download', (k) =>
					this._OnInvokeDownload(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'raster-svg-image', (k) =>
					this._OnRasterSvgImage(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'get-svg-image-size', (k) =>
					this._OnGetSvgImageSize(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'set-target-orientation', (k) =>
					this._OnSetTargetOrientation(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'register-sw', () => this._OnRegisterSW());
				f.AddRuntimeComponentMessageHandler('runtime', 'post-to-debugger', (k) =>
					this._OnPostToDebugger(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'go-to-script', (k) =>
					this._OnPostToDebugger(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'before-start-ticking', () =>
					this._OnBeforeStartTicking()
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'debug-highlight', (k) =>
					this._OnDebugHighlight(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'enable-device-orientation', () =>
					this._AttachDeviceOrientationEvent()
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'enable-device-motion', () =>
					this._AttachDeviceMotionEvent()
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'add-stylesheet', (k) =>
					this._OnAddStylesheet(k)
				);
				f.AddRuntimeComponentMessageHandler('runtime', 'alert', (k) => this._OnAlert(k));
				f.AddRuntimeComponentMessageHandler('runtime', 'hide-cordova-splash', () =>
				...
				c.addEventListener('selectstart', x);
				c.addEventListener('gesturehold', x);
				window.addEventListener('touchstart', x, { passive: !1 });
				'undefined' !== typeof PointerEvent
					? (window.addEventListener('pointerdown', x, { passive: !1 }),
					  c.addEventListener('pointerdown', x))
					: c.addEventListener('touchstart', x);
				...
				window.addEventListener('wheel', C, { passive: !1 });
			...
			_OnBeforeStartTicking() {
				'cordova' === this._iRuntime.GetExportType()
					? (document.addEventListener('pause', () => this._OnVisibilityChange(!0)),
					  document.addEventListener('resume', () => this._OnVisibilityChange(!1)))
					: document.addEventListener('visibilitychange', () =>
							this._OnVisibilityChange(document.hidden)
					  );
				return { isSuspended: !(!document.hidden && !D) };
			}
			Attach() {
				window.addEventListener('focus', () => this._PostRuntimeEvent('window-focus'));
				window.addEventListener('blur', () => {
					this._PostRuntimeEvent('window-blur', { parentHasFocus: E() });
					this._mousePointerLastButtons = 0;
				});
				...
				'undefined' !== typeof PointerEvent
					? (window.addEventListener('pointerdown', (g) => {
					  ...
					  self === self.top
							? ((this._pointerRawUpdateRateLimiter = new self.RateLimiter(
							  ...
							  window.addEventListener('pointerrawupdate', (g) => this._OnPointerRawUpdate(g)))
							: window.addEventListener('pointermove', (g) =>
									this._OnPointerEvent('pointermove', g)
							  ),
					  window.addEventListener('pointerup', (g) => this._OnPointerEvent('pointerup', g)),
					  window.addEventListener('pointercancel', (g) =>
							this._OnPointerEvent('pointercancel', g)
					  ))
					: (window.addEventListener('mousedown', (g) => {
					  ...
					  window.addEventListener('mousemove', (g) =>
							this._OnMouseEventAsPointer('pointermove', g)
					  ),
					  window.addEventListener('mouseup', (g) => this._OnMouseEventAsPointer('pointerup', g)),
					  window.addEventListener('touchstart', (g) => {
							this._HandlePointerDownFocus(g);
							this._OnTouchEvent('pointerdown', g);
					  }),
					  window.addEventListener('touchmove', (g) => this._OnTouchEvent('pointermove', g)),
					  window.addEventListener('touchend', (g) => this._OnTouchEvent('pointerup', g)),
					  window.addEventListener('touchcancel', (g) => this._OnTouchEvent('pointercancel', g)));
			...
			_OnSimulatedResize(f, g, c) {
				...
				k != f || l != g
					...
					: 10 > c && this._ScheduleSimulatedResize(k, l, c + 1);
			}
			...
			_TrySetTargetOrientation() {
				...
				if (screen.orientation && screen.orientation.lock)
					screen.orientation
						.lock(f)
						.catch((g) => console.warn('[Construct 3] Failed to lock orientation: ', g));
				else
					try {
						let g = !1;
						screen.lockOrientation
							? (g = screen.lockOrientation(f))
							: screen.webkitLockOrientation
							? (g = screen.webkitLockOrientation(f))
							: screen.mozLockOrientation
							? (g = screen.mozLockOrientation(f))
							: screen.msLockOrientation && (g = screen.msLockOrientation(f));
			...
			_OnVisibilityChange(f) {
				...
				this.PostToRuntime('visibilitychange', { hidden: f });
			}
			_OnKeyEvent(f, g) {
				'Backspace' === g.key && x(g);
			...
			_OnMouseEventAsPointer(f, g) {
				if (!a(g)) {
					...
					'pointerdown' === f && 0 !== c
						? (f = 'pointermove')
						: 'pointerup' === f && 0 !== g.buttons && (f = 'pointermove');
					this._PostToRuntimeMaybeSync(
						f,
						{
							pointerId: 1,
			...
			_OnPointerEvent(f, g) {
				...
				'mouse' === g.pointerType &&
					((c = 'mousemove'),
					'pointerdown' === f ? (c = 'mousedown') : 'pointerup' === f && (c = 'mouseup'),
					this._OnMouseEvent(c, g, h),
					(this._mousePointerLastButtons = g.buttons));
			}
			...
			_HandlePointerDownFocus(f) {
				...
				this._IsElementCanvasOrDocument(f.target) &&
			...
			_OnDeviceMotion(f) {
				...
				c && (g = { x: c.x || 0, y: c.y || 0, z: c.z || 0 });
				c = null;
				var k = f.accelerationIncludingGravity;
				k && (c = { x: k.x || 0, y: k.y || 0, z: k.z || 0 });
				k = null;
				const l = f.rotationRate;
				l && (k = { alpha: l.alpha || 0, beta: l.beta || 0, gamma: l.gamma || 0 });
				this.PostToRuntime(
					'devicemotion',
					{
						acceleration: g,
			...
			_OnUpdateCanvasSize(f) {
				const g = this.GetRuntimeInterface().GetCanvas();
				g.style.width = f.styleWidth + 'px';
				g.style.height = f.styleHeight + 'px';
				g.style.marginLeft = f.marginLeft + 'px';
				g.style.marginTop = f.marginTop + 'px';
				this._isFirstSizeUpdate && ((g.style.display = ''), (this._isFirstSizeUpdate = !1));
			}
			_OnInvokeDownload(f) {
				const g = f.url;
				f = f.filename;
			...
			async _OnRasterSvgImage(f) {
				...
				f = await self.C3_RasterSvgImageBlob(
					f.blob,
					f.imageWidth,
					f.imageHeight,
					f.surfaceWidth,
				...
				return { imageBitmap: g, transferables: [g] };
			}
			...
			_InvokeFunctionFromJS(f, g) {
				return this.PostToRuntimeAsync('js-invoke-function', { name: f, params: g });
			}
			_OnAlert(f) {
				alert(f.message);
			}
...
self.JobSchedulerDOM = class {
	constructor(d) {
		...
		'preview' === d.GetExportType()
			? (this._baseUrl += 'c3/workers/')
			: (this._baseUrl += d.GetScriptFolder());
	...
	async Init() {
		...
		this._dispatchWorker = await this._runtimeInterface.CreateWorker(d, this._baseUrl, {
			name: 'DispatchWorker'
		});
		d = new MessageChannel();
		this._inputPort = d.port1;
		this._dispatchWorker.postMessage({ type: '_init', 'in-port': d.port2 }, [d.port2]);
	...
	async _CreateJobWorker() {
		...
		a = await this._runtimeInterface.CreateWorker(a, this._baseUrl, { name: 'JobWorker' + d });
		const b = new MessageChannel(),
			e = new MessageChannel();
		this._dispatchWorker.postMessage({ type: '_addJobWorker', port: b.port1 }, [b.port1]);
		a.postMessage({ type: 'init', number: d, 'dispatch-port': b.port2, 'output-port': e.port2 }, [
			b.port2,
			e.port2
		]);
...
('use strict');
{
	...
	self.AudioDOMHandler = class extends self.DOMHandler {
		...
		async _CreateAudioContext(a) {
			...
			try {
				await Promise.all(
					a.preloadList.map((e) => this._GetAudioBuffer(e.originalUrl, e.url, e.type, !1))
				);
			} catch (e) {
		...
		_ReconnectEffects(a) {
			...
			if (e && e.length) {
				...
				for (let h = 0, m = e.length; h < m; ++h) {
					const p = e[h];
					h + 1 === m
						? p.ConnectTo(this.GetDestinationNode())
						: p.ConnectTo(e[h + 1].GetInputNode());
				}
			}
		...
		DecodeAudioData(a, b) {
			return b
				? this._iRuntime._WasmDecodeWebMOpus(a).then((e) => {
		...
		*audioInstancesByTag(a) {
		...
		async _GetAudioBuffer(a, b, e, h, m) {
			for (const p of this._audioBuffers) if (p.GetUrl() === b) return await p.Load(), p;
			if (m) return null;
			h &&
		...
		async _GetAudioInstance(a, b, e, h, m) {
			for (const p of this._audioInstances)
				if (p.GetUrl() === b && (p.CanBeRecycled() || m)) return p.SetTag(h), p;
		...
		_AddPendingTag(a) {
			...
			if (!b) {
				let e = null;
				b = { pendingCount: 0, promise: new Promise((h) => (e = h)), resolve: e };
		...
		PostTrigger(a, b, e) {
			this.PostToRuntime('trigger', { type: a, tag: b, aiid: e });
		}
		async _Play(a) {
			...
			0 < w &&
				...
				(this._audioContext.getOutputTimestamp
					? ((a = this._audioContext.getOutputTimestamp()),
					  (w = w - a.performanceTime / 1e3 + a.contextTime))
					: (w = w - performance.now() / 1e3 + this._audioContext.currentTime));
			this._lastPlayedTag = p;
			this._AddPendingTag(p);
			try {
				(this._lastAudioInstance = await this._GetAudioInstance(b, e, h, p, m)),
					u
						? (this._lastAudioInstance.SetPannerEnabled(!0),
						  ...
						  u.hasOwnProperty('uid') && this._lastAudioInstance.SetUID(u.uid))
						: this._lastAudioInstance.SetPannerEnabled(!1),
					this._lastAudioInstance.Play(v, z, A, w);
			} catch (B) {
		...
		async _OnLoadState(a) {
			...
			if (3 !== b)
				for (var e of this._audioInstances)
					(e.IsMusic() && 1 === b) || ((e.IsMusic() || 2 !== b) && e.Stop());
			for (const h of this._effects.values()) for (const m of h) m.Release();
			this._effects.clear();
			this._timeScale = a.timeScale;
			this._gameTime = a.gameTime;
			e = a.listenerPos;
			...
			for (const h of Object.values(a.effects))
				e.push(Promise.all(h.map((m) => this._AddEffect(m))));
			await Promise.all(e);
			...
			if (3 !== b) {
				...
				if (!p || 1 !== b)
					if (p || 2 !== b) {
						b = null;
						try {
							b = await this._GetAudioInstance(e, h, m, v, p);
						} catch (w) {
...
self.C3AudioBuffer = class {
	...
	static Create(d, a, b, e, h) {
		const m = 'audio/webm; codecs=opus' === e && !d.SupportsWebMOpus();
		h && m && d._SetHasAnySoftwareDecodedMusic();
		return !h || d.IsPlayMusicAsSound() || m
			? new self.C3WebAudioBuffer(d, a, b, e, h, m)
			: new self.C3Html5AudioBuffer(d, a, b, e, h);
	}
	CreateInstance(d) {
		return 'html5' === this._api
			? new self.C3Html5AudioInstance(this._audioDomHandler, this, d)
			: new self.C3WebAudioInstance(this._audioDomHandler, this, d);
	}
	_Load() {}
	Load() {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
	...
	async _Fetch() {
		...
		if (
			'cordova' === d.GetExportType() &&
			d.IsRelativeURL(this._url) &&
			'file:' === location.protocol
		)
			this._audioData = await d.CordovaFetchLocalFileAsArrayBuffer(this._url);
		else {
	...
	async _Load() {
		try {
			...
			(this._loadState = 'failed'),
				console.error(`[Construct 3] Failed to load audio '${this._url}': `, d);
		}
	}
	IsLoaded() {
	...
	self.C3AudioInstance = class {
		...
		SetPanXYA(a, b, e) {
			this._isPannerEnabled &&
				...
				(this._pannerOrientation[0] = Math.cos(e)),
				(this._pannerOrientation[1] = Math.sin(e)),
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
	...
	Play(d, a, b, e) {
		...
		if (e.currentTime !== b)
			try {
				...
				console.warn(
					`[Construct 3] Exception seeking audio '${this._buffer.GetUrl()}' to position '${b}': `,
					h
				);
			}
	...
	Resume() {
		!this._isPaused ||
			this._isStopped ||
			this.HasEnded() ||
			(this._audioDomHandler.TryPlayMedia(this.GetAudioElement()), (this._isPaused = !1));
	}
	_UpdateMuted() {
	...
	Seek(d) {
		if (!this._isStopped && !this.HasEnded())
			try {
				...
				console.warn(`[Construct 3] Error seeking audio to '${d}': `, a);
			}
	}
	GetResumePosition() {
	...
	SetSuspended(d) {
		d
			? this.IsPlaying()
				...
				: (this._resumeMe = !1)
			: this._resumeMe &&
			  (this._audioDomHandler.TryPlayMedia(this.GetAudioElement()), (this._resumeMe = !1));
	}
};
('use strict');
self.C3WebAudioInstance = class extends self.C3AudioInstance {
	...
	HasEnded() {
		return (!this._isStopped && this._bufferSource && this._bufferSource.loop) || this._isPaused
			? !1
	...
	Resume() {
		!this._isPaused ||
			...
			(this._startTime =
				this.GetCurrentTime() - this._resumePosition / (this._playbackRate || 0.001)),
	...
	SetLooping(d) {
		d = !!d;
		this._isLooping !== d &&
			((this._isLooping = d), this._bufferSource && (this._bufferSource.loop = d));
	}
	_UpdatePlaybackRate() {
		let d = this._playbackRate;
		this._isTimescaled && (d *= this._audioDomHandler.GetTimeScale());
	...
	Seek(d) {
		this._isStopped ||
			this.HasEnded() ||
			(this._isPaused
				? (this._resumePosition = d)
				: (this.Pause(), (this._resumePosition = d), this.Resume()));
	}
	GetResumePosition() {
...
('use strict');
{
	...
	self.C3AudioFilterFX = class extends d {
		constructor(a, b, e, h, m, p, v) {
			super(a);
			this._type = 'filter';
			this._params = [b, e, h, m, p, v];
	...
	class extends self.DOMHandler {
		...
		async _OnRequestPermission(d) {
			...
			0 === d
				...
				: 1 === d && (a = await this._RequestMotionPermission());
			this.PostToRuntime('permission-result', { type: d, result: a });
		}
	...
	class extends self.DOMHandler {
		...
		_OnReadyForSWMessages() {
			window.C3_RegisterSW &&
				...
				window.OfflineClientInfo.SetMessageCallback((d) =>
					this.PostToRuntime('sw-message', d.data)
				);
		}
		_OnOnlineStateChanged(d) {
			this.PostToRuntime('online-state', { isOnline: d });
		}
		...
		_OnLockOrientation(d) {
			...
			if (screen.orientation && screen.orientation.lock)
				screen.orientation
					.lock(d)
					.catch((a) => console.warn('[Construct 3] Failed to lock orientation: ', a));
			else
				try {
					let a = !1;
					screen.lockOrientation
						? (a = screen.lockOrientation(d))
						: screen.webkitLockOrientation
						? (a = screen.webkitLockOrientation(d))
						: screen.mozLockOrientation
						? (a = screen.mozLockOrientation(d))
						: screen.msLockOrientation && (a = screen.msLockOrientation(d));
		...
		_OnNavigate(d) {
			...
			if ('back' === a)
				...
				var b = d.target;
				d = d.exportType;
				'windows-uwp' === d && 'undefined' !== typeof Windows
				...
				'new-window' === a &&
					((a = d.url),
					(b = d.tag),
					'windows-uwp' === d.exportType && 'undefined' !== typeof Windows
						? Windows.System.Launcher.launchUriAsync(new Windows.Foundation.Uri(a))
						: self.cordova && self.cordova.InAppBrowser
						? self.cordova.InAppBrowser.open(a, '_system')
						: window.open(a, b));
		...
		_OnRequestFullscreen(d) {
			...
			d.requestFullscreen
				? d.requestFullscreen(a)
				: d.mozRequestFullScreen
				? d.mozRequestFullScreen(a)
				: d.msRequestFullscreen
				? d.msRequestFullscreen(a)
